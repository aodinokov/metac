...
int metac_unknown_object_supports_snprintf(
    ...
    if (p_unknown_object->p_unknown_object_ops == NULL
            || p_unknown_object->p_unknown_object_ops->metac_unknown_object_snprintf
                    == NULL) {
...
int metac_unknown_object_snprintf(struct metac_unknown_object *p_unknown_object,
...
int metac_unknown_object_supports_delete(
    ...
    if (p_unknown_object->p_unknown_object_ops == NULL
            || p_unknown_object->p_unknown_object_ops->metac_unknown_object_delete
                    == NULL) {
        msg_stddbg("Unsupported operation in ops: %p\n",
                p_unknown_object->p_unknown_object_ops);
        return 0;
...
int metac_unknown_object_delete(struct metac_unknown_object *p_unknown_object) {
...
int metac_refcounter_object_init(
    ...
    if (metac_unknown_object_init(&p_refcounter_object->unknown_object,
            &p_refcounter_object_ops->unknown_object_ops, p_private_data)
            != 0) {
...
metac_refcounter_object_get(struct metac_refcounter_object *p_refcounter_object) {
    ...
    if (p_refcounter_object->ref_count == 0) {
        msg_stderr(
                "Requested for reference increment object %p had to be deleted before but wasn't deleted. This process has memory leaks\n",
                p_refcounter_object);
        return NULL;
    }
...
int metac_refcounter_object_put(
    ...
    if (p_refcounter_object->ref_count == 0) {
        msg_stderr(
                "Requested for deletion object %p had to be deleted before but wasn't deleted. This process has memory leaks\n",
                p_refcounter_object);
        return (-EFAULT);
    }
    ...
    if (metac_unknown_object_delete(&p_refcounter_object->unknown_object)
            != 0) {
        msg_stderr(
                "Can't delete object %p, still removing reference. We are stopping keep track of it's references. This process has memory leaks\n",
                p_refcounter_object);
        return -(EFAULT);
    }
