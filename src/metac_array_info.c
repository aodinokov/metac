...
metac_array_info_t* metac_array_info_create_from_type(struct metac_type *type,
    ...
    p_array_info = calloc(1,
            sizeof(*p_array_info)
                    + subranges_count
                            * sizeof(struct _metac_array_subrange_info));
...
metac_array_info_t* metac_array_info_create_from_elements_count(
...
int metac_array_info_get_subrange_count(metac_array_info_t *p_array_info,
        metac_num_t subrange, metac_num_t *p_count) {
    if (p_array_info == NULL) {
...
int metac_array_info_set_subrange_count(metac_array_info_t *p_array_info,
        metac_num_t subrange, metac_num_t count) {
    if (p_array_info == NULL) {
        msg_stderr("invalid argument value: p_array_info\n");
        return (-EINVAL);
    }
...
metac_array_info_t* metac_array_info_copy(metac_array_info_t *p_array_info_orig) {
    ...
    for (i = 0; i < p_array_info->subranges_count; ++i) {
        p_array_info->subranges[i].count =
                p_array_info_orig->subranges[i].count;
    }
...
metac_count_t metac_array_info_get_element_count(
        metac_array_info_t *p_array_info) {
    metac_num_t i;
...
int metac_array_info_is_equal(metac_array_info_t *p_array_info0,
        metac_array_info_t *p_array_info1) {
    metac_num_t i;

    if (p_array_info0 == NULL || p_array_info1 == NULL) {
        return -EINVAL;
    }
    ...
    for (i = 0; i < p_array_info0->subranges_count; ++i) {
        if (p_array_info0->subranges[i].count
                != p_array_info1->subranges[i].count)
            return 0;
    }

    return 1;
}
metac_array_info_t* metac_array_info_create_counter(
        metac_array_info_t *p_array_info) {
    metac_num_t i;
    metac_num_t subranges_count;
    ...
    p_array_info_counter = calloc(1,
            sizeof(*p_array_info_counter)
                    + subranges_count
                            * sizeof(struct _metac_array_subrange_info));
...
int metac_array_info_increment_counter(metac_array_info_t *p_array_info,
        metac_array_info_t *p_array_info_counter) {
    metac_num_t i;
    if (p_array_info == NULL || p_array_info_counter == NULL
            || p_array_info->subranges_count
                    != p_array_info_counter->subranges_count) {
    ...
    while (1) {
        ...
        if (p_array_info_counter->subranges[i].count
                == p_array_info->subranges[i].count) {
            p_array_info_counter->subranges[i].count = 0;
            if (i == 0)
                return 1; /*overflow*/
...
metac_array_info_t* metac_array_info_convert_linear_id_2_subranges(
        metac_array_info_t *p_array_info, metac_num_t linear_id) {
    int i;
    ...
    for (i = 1; i <= p_array_info->subranges_count; ++i) {
        p_subranges_id->subranges[p_array_info->subranges_count - i].count =
                linear_id
                        % p_array_info->subranges[p_array_info->subranges_count
                                - i].count;
        linear_id =
                linear_id
                        / p_array_info->subranges[p_array_info->subranges_count
