...
static struct memory_backend_json* memory_backend_json_create(
        json_object *p_master_json, json_object *p_current_object_json);
int memory_backend_interface_create_from_json(json_object *p_master_json,
        struct memory_backend_interface **pp_memory_backend_interface);
int memory_backend_interface_create_from_json_ex(json_object *p_master_json,
...
static json_object* _get_root(json_object *p_master_json) {
json_object *p_json_root;

if (json_object_object_get_ex(p_master_json, "root", &p_json_root) == 0) {
...
static json_object* _get_pointer_destination(json_object *p_json_pointer_object,
    json_object *p_json_root) {
json_object *p_current_object_json;
json_object *p_json_json_path;
const char *json_path;

if (json_object_object_get_ex(p_json_pointer_object, "json_path",
        &p_json_json_path) == 0) {
...
if (json_pointer_get(p_json_root, json_path, &p_current_object_json) != 0) {
...
return p_memory_backend_json0->p_master_json
        == p_memory_backend_json0->p_master_json
        && p_memory_backend_json0->p_current_object_json
                == p_memory_backend_json1->p_current_object_json;
}
/*****************************************************************************/
static int _element_get_memory_backend_interface(struct element *p_element,
    struct memory_backend_interface **pp_memory_backend_interface) {
...
p_memory_backend_json = memory_backend_json(
        p_element->p_memory_block->p_memory_backend_interface);
assert(p_memory_backend_json);

p_array_indexes = metac_array_info_convert_linear_id_2_subranges(
        p_element->p_memory_block->p_array_info, p_element->id);
if (p_array_indexes == NULL) {
    msg_stderr("metac_array_info_convert_linear_id_2_subranges failed\n");
    return -(EFAULT);
}

p_current_object_json = json_object_get(
...
for (i = 0; i < p_array_indexes->subranges_count; ++i) {
    ...
    p_element_object_json = json_object_array_get_idx(p_current_object_json,
            p_array_indexes->subranges[i].count);
    json_object_put(p_current_object_json);
...
p_result_memory_backend_json = memory_backend_json_create(
        p_memory_backend_json->p_master_json, p_current_object_json);
json_object_put(p_current_object_json);
...
if (pp_memory_backend_interface != NULL) {
    *pp_memory_backend_interface =
            &p_memory_backend_json->memory_backend_interface;
}

return 0;
}

static int _element_read_memory_backend_interface(struct element *p_element,
    struct memory_backend_interface **pp_memory_backend_interface) {
...
if (_element_get_memory_backend_interface(p_element,
...
p_current_object_json = _get_pointer_destination(
        p_memory_backend_json->p_current_object_json, p_json_root);
...
static int _element_get_array_subrange0(struct element *p_element) {
...
p_element->array.subrange0_count = json_object_array_length(
        p_memory_backend_json->p_current_object_json);
...
static int _element_get_pointer_subrange0(struct element *p_element) {

struct memory_backend_json *p_memory_backend_json;
struct memory_backend_interface *p_memory_backend_interface;

if (_element_read_memory_backend_interface(p_element,
        &p_memory_backend_interface) != 0) {
...
/* p_memory_backend_json->p_current_object_json contains array */
p_element->pointer.subrange0_count = json_object_array_length(
        p_memory_backend_json->p_current_object_json);
...
static int _element_cast_pointer(struct element *p_element) {
...
p_element->pointer.p_casted_element_type =
        p_element->p_element_type->pointer.p_element_type;

if (json_object_object_get_ex(p_memory_backend_json->p_current_object_json,
        "cast_id", &json_object_cast_id) == 0) {
    /*not casted*/
    memory_backend_interface_get(
            p_element->pointer.p_original_memory_backend_interface,
            &p_element->pointer.p_casted_memory_backend_interface);
    memory_backend_interface_put(&p_memory_backend_interface);
    return 0;
}
...
p_element->pointer.generic_cast_type_id = json_object_get_int(
        json_object_cast_id);
json_object_put(json_object_cast_id);

if (p_element->pointer.generic_cast_type_id
        >= p_element->p_element_type->pointer.generic_cast.types_count) {
    msg_stderr(
            "generic_cast.cb set incorrect generic_cast_type_id %d. Maximum value is %d\n",
...
p_element->pointer.p_casted_element_type =
        p_element->p_element_type->pointer.generic_cast.p_types[p_element->pointer.generic_cast_type_id].p_element_type;
...
p_element->pointer.casted_based_original_offset = json_object_get_int(
        json_object_casted_based_original_offset);
...
/*****************************************************************************/
static int _get_hierarchy_member_related_json(json_object *p_json_root,
...
while (p_current_hierarchy_member != NULL) {
    ...
    if (p_current_hierarchy_member->p_member_info->name != NULL
            && strcmp(p_current_hierarchy_member->p_member_info->name,
                    METAC_ANON_MEMBER_NAME) != 0) {
        ++count;
    }

    p_current_hierarchy_member =
...
while (p_current_hierarchy_member != NULL) {
    ...
    if (p_current_hierarchy_member->p_member_info->name != NULL
        ...
        p_names[count - i - 1] =
                p_current_hierarchy_member->p_member_info->name;
        ++i;
    }
...
static int _guess_discriminator_value(struct element_type *p_element_type,
    struct element_type_hierarchy *p_hierarchy, json_object *p_hierarchy_json,
    metac_discriminator_value_t *p_value) {
...
for (i = 0; i < p_element_type->hierarchy_top.members_count; ++i) {
    ...
    if (_is_hierarchy_closest_non_anon_member(p_hierarchy,
            p_element_type->hierarchy_top.pp_members[i], &current_value) == 1) {

        /*check if exists*/
        p_json =
                json_object_object_get(p_hierarchy_json,
                        p_element_type->hierarchy_top.pp_members[i]->p_member_info->name);
        if (p_json != NULL) { /*found the field*/
            ...
            if (value_is_set != 0) {
                if (value != current_value) {
                    json_object_put(p_json);
                    msg_stderr(
                            "wasn't able to identify discriminator value, because of ambiguity: already had %d, but now getting %d\n",
 ...
 */
for (i = 0; i < p_element->p_element_type->hierarchy_top.discriminators_count;
        ++i) {
    ...
    p_hierarchy_member =
            element_type_hierarchy_get_element_hierarchy_member(
    ...
    if (p_hierarchy_member == NULL) {
        ...
        p_hierarchy_json = json_object_get(
                p_memory_backend_json->p_current_object_json);
    } else {
        ...
        result = _get_hierarchy_member_related_json(
                p_memory_backend_json->p_current_object_json,
                p_hierarchy_member, &p_hierarchy_json);
     ...
     */
    if (_guess_discriminator_value(p_element->p_element_type,
            p_element->p_element_type->hierarchy_top.pp_discriminators[i]->p_hierarchy,
            p_hierarchy_json,
...
assert(
        p_element_hierarchy_member->p_element->p_memory_block->p_memory_backend_interface);

if (_element_get_memory_backend_interface(p_element_hierarchy_member->p_element,
...
if (_element_hierarchy_member_get_memory_backend_interface(
        p_element_hierarchy_member, &p_memory_backend_interface) != 0) {
    msg_stderr(
            "_element_hierarchy_member_get_memory_backend_interface failed\n");
    return -(EFAULT);
}
...
p_element_hierarchy_member->array.subrange0_count = json_object_array_length(
        p_memory_backend_json->p_current_object_json);
...
if (_element_hierarchy_member_read_memory_backend_interface(
        p_element_hierarchy_member, &p_memory_backend_interface) != 0) {
    msg_stderr(
            "_element_hierarchy_member_read_memory_backend_interface failed\n");
    return -(EFAULT);
}
...
/* p_memory_backend_json->p_current_object_json contains array */
p_element_hierarchy_member->pointer.subrange0_count = json_object_array_length(
        p_memory_backend_json->p_current_object_json);
...
p_element_hierarchy_member->pointer.p_casted_element_type =
        p_element_hierarchy_member->p_element_type_hierarchy_member->pointer.p_element_type;
...
if (json_object_object_get_ex(p_memory_backend_json->p_current_object_json,
    ...
    memory_backend_interface_get(
            ...
            &p_element_hierarchy_member->pointer.p_casted_memory_backend_interface);
    memory_backend_interface_put(&p_memory_backend_interface);
    return 0;
}
...
/*TDOO: check json_object_cast_id type*/
p_element_hierarchy_member->pointer.generic_cast_type_id = json_object_get_int(
        json_object_cast_id);
json_object_put(json_object_cast_id);

if (p_element_hierarchy_member->pointer.generic_cast_type_id
...
p_element_hierarchy_member->pointer.p_casted_element_type =
        p_element_hierarchy_member->p_element_type_hierarchy_member->pointer.generic_cast.p_types[p_element_hierarchy_member->pointer.generic_cast_type_id].p_element_type;
...
/*TDOO: check json_object_casted_based_original_offset type*/
p_element_hierarchy_member->pointer.casted_based_original_offset =
        json_object_get_int(json_object_casted_based_original_offset);
...
.memory_backend_interface_delete = _memory_backend_interface_delete,
    ...
    .element_get_memory_backend_interface =
            _element_get_memory_backend_interface,
    .element_read_memory_backend_interface =
            _element_read_memory_backend_interface,
    ...
    .element_hierarchy_member_get_array_subrange0 =
            _element_hierarchy_member_get_array_subrange0,
    .element_hierarchy_member_get_pointer_subrange0 =
            _element_hierarchy_member_get_pointer_subrange0,
    .element_hierarchy_member_cast_pointer =
            _element_hierarchy_member_cast_pointer,
...
memory_backend_interface(&p_memory_backend_json->memory_backend_interface,
        &ops);
p_memory_backend_json->p_master_json = json_object_get(p_master_json);
p_memory_backend_json->p_current_object_json = json_object_get(
...
if (override_current_object_json_path == NULL) {
    ...
    if (json_object_object_get_ex(p_master_json, "entry_point",
    ...
    p_current_object_json = _get_pointer_destination(p_json_pointer_object,
            p_json_root);
    ...
    if (json_pointer_get(p_json_root, override_current_object_json_path,
            &p_current_object_json) != 0) {
        msg_stderr("wasn't able to find object by path %s\n",
...
p_memory_backend_json = memory_backend_json_create(p_master_json,
        p_current_object_json);
json_object_put(p_current_object_json);
...
int memory_backend_interface_create_from_json(json_object *p_master_json,
    struct memory_backend_interface **pp_memory_backend_interface) {
return memory_backend_interface_create_from_json_ex(p_master_json, NULL,
        pp_memory_backend_interface);
}
