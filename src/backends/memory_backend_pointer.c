...
static int _memory_backend_interface_create_from_pointer(void *ptr,
...
struct memory_backend_pointer *p_original_memory_backend_pointer =
        memory_backend_pointer(
...
if (p_element_type_pointer->generic_cast.cb != NULL) {
    int res = p_element_type_pointer->generic_cast.cb(
            p_element_type_pointer->annotation_key, 0,
            &p_element_pointer->use_cast,
            &p_element_pointer->generic_cast_type_id, &p_original, &p_casted,
            p_element_type_pointer->generic_cast.p_data);
    if (res != 0) {
        msg_stderr("generic_cast.cb returned error %d\n", res);
        return res;
    }
    if (p_element_pointer->generic_cast_type_id
    ...
    p_element_pointer->p_casted_element_type =
            p_element_type_pointer->generic_cast.p_types[p_element_pointer->generic_cast_type_id].p_element_type;
}
...
if (_memory_backend_interface_create_from_pointer(p_casted,
...
struct memory_backend_pointer *p_memory_backend_pointer =
        memory_backend_pointer(p_element_array->p_memory_backend_interface);
void *ptr;
...
if (p_element_type_pointer->array_elements_count.cb(
        p_element_type_pointer->annotation_key, 0, ptr,
        p_element_type_pointer->p_element_type->p_actual_type,
        /*TODO: take the next params from the global context*/
        /*FIXME:*/NULL,
        /*FIXME:*/NULL, &p_element_pointer->subrange0_count,
...
if ((*pp_memory_backend_interface) != NULL) {
    p_memory_backend_pointer = memory_backend_pointer(
...
if (_memory_backend_interface_create_from_pointer(
        *((void**) (p_memory_backend_pointer->ptr
                + p_element->id * p_element->p_element_type->byte_size)),
        pp_memory_backend_interface) != 0) {
    msg_stderr("_memory_backend_interface_create_from_pointer failed\n");
    return -(EFAULT);
...
return element_array_get_array_subrange0(&p_element->array,
...
return element_pointer_get_pointer_subrange0(&p_element->pointer,
...
return element_pointer_cast_pointer(&p_element->pointer,
        &p_element->p_element_type->pointer);
}
...
for (i = 0; i < p_element->p_element_type->hierarchy_top.discriminators_count;
    ...
    if (p_element->hierarchy_top.pp_discriminator_values[i]) {
        if (p_element->p_element_type->hierarchy_top.pp_discriminators[i]->cb(
                ...
                p_element->p_element_type->hierarchy_top.pp_discriminators[i]->p_data)
                < 0) {
...
if (_memory_backend_interface_create_from_pointer(
        p_memory_backend_pointer->ptr
                + p_element_hierarchy_member->p_element->id
                        * p_element_hierarchy_member->p_element->p_element_type->byte_size
...
return element_array_get_array_subrange0(&p_element_hierarchy_member->array,
...
return element_pointer_cast_pointer(&p_element_hierarchy_member->pointer,
        &p_element_hierarchy_member->p_element_type_hierarchy_member->pointer);
...
static int _object_root_validate(struct object_root *p_object_root) {
...
_tops = (struct memory_block_top**) calloc(
        p_object_root->memory_block_tops_count, sizeof(*_tops));
if (_tops == NULL) {
...
/*find unique regions*/
qsort(_tops, p_object_root->memory_block_tops_count, sizeof(*_tops),
        memory_block_top_compare4qsort); /*TODO: change to hsort? - low prio so far*/

for (i = 1; i < p_object_root->memory_block_tops_count; ++i) {
    if (memory_block_top_get_ptr(_tops[i - 1])
            == memory_block_top_get_ptr(_tops[i])) {
        msg_stderr("found the same ptr twice- error\n");
        result = -EFAULT;
        break;
    }
    if (memory_block_top_get_ptr(_tops[i])
            > memory_block_top_get_ptr(_tops[i - 1])
            && memory_block_top_get_ptr(_tops[i])
                    < memory_block_top_get_ptr(_tops[i - 1])
                            + _tops[i - 1]->memory_block.byte_size) {
...
memory_backend_interface(&p_memory_backend_pointer->memory_backend_interface,
        &ops);
