...
int memory_backend_interface_put(
    ...
    if ((*pp_memory_backend_interface)->p_ops->memory_backend_interface_delete
            == NULL) {
        msg_stderr("memory_backend_interface_delete isn't defined\n");
        return -(ENOENT);
    }

    return (*pp_memory_backend_interface)->p_ops->memory_backend_interface_delete(
...
int memory_backend_interface_equals(
    ...
    if (p_memory_backend_interface0->p_ops
            != p_memory_backend_interface1->p_ops) {
        msg_stderr("backend interfaces have different types\n");
        return -(EINVAL);
    }

    if (p_memory_backend_interface0->p_ops->memory_backend_interface_equals
            == NULL) {
    ...
    return p_memory_backend_interface0->p_ops->memory_backend_interface_equals(
            p_memory_backend_interface0, p_memory_backend_interface1);
}

int element_get_memory_backend_interface(struct element *p_element,
        struct memory_backend_interface **pp_memory_backend_interface) {
    struct memory_backend_interface *p_memory_backend_interface;
    ...
    if (p_memory_backend_interface->p_ops->element_get_memory_backend_interface
            == NULL) {
    ...
    return p_memory_backend_interface->p_ops->element_get_memory_backend_interface(
            p_element, pp_memory_backend_interface);
}

int element_read_memory_backend_interface(struct element *p_element,
        struct memory_backend_interface **pp_memory_backend_interface) {
    struct memory_backend_interface *p_memory_backend_interface;
    ...
    if (p_memory_backend_interface->p_ops->element_read_memory_backend_interface
            == NULL) {
        msg_stderr("element_get_memory_backend_interface isn't defined\n");
        return -(ENOENT);
    }
...
int element_get_array_subrange0(struct element *p_element) {
    struct memory_backend_interface *p_memory_backend_interface;
    ...
    if (p_memory_backend_interface->p_ops->element_get_array_subrange0
            == NULL) {
...
int element_get_pointer_subrange0(struct element *p_element) {
    struct memory_backend_interface *p_memory_backend_interface;
    ...
    if (p_memory_backend_interface->p_ops->element_get_pointer_subrange0
            == NULL) {
...
int element_cast_pointer(struct element *p_element) {
    ...
    if (p_memory_backend_interface->p_ops->element_cast_pointer(p_element)
            != 0) {
    ...
    if (p_element->pointer.p_casted_element_type == NULL
        ...
        msg_stderr(
                "element_cast_pointer call was successful, but some mandatory fields were still empty\n");
        return -(EFAULT);
    }
...
int element_calculate_hierarchy_top_discriminator_values(
    ...
    if (p_element->p_element_type->p_actual_type->id != DW_TAG_structure_type
            && p_element->p_element_type->p_actual_type->id
                    != DW_TAG_union_type) {
    ...
    if (p_memory_backend_interface->p_ops->element_calculate_hierarchy_top_discriminator_values
            == NULL) {
...
int element_hierarchy_member_get_memory_backend_interface(
    ...
    if (p_element_hierarchy_member->p_element->id
            >= p_element_hierarchy_member->p_element->p_memory_block->elements_count) {
        msg_stderr("id %d is invalid. elements_count is %d\n",
    ...
    if (p_memory_backend_interface->p_ops->element_hierarchy_member_get_memory_backend_interface
            == NULL) {
        msg_stderr(
                "element_hierarchy_member_get_memory_backend_interface isn't defined\n");
        return -(ENOENT);
    }

    return p_memory_backend_interface->p_ops->element_hierarchy_member_get_memory_backend_interface(
            p_element_hierarchy_member, pp_memory_backend_interface);
}
...
int element_hierarchy_member_read_memory_backend_interface(
    ...
    if (p_element_hierarchy_member->p_element_type_hierarchy_member->p_actual_type->id
            != DW_TAG_pointer_type) {
    ...
    if (p_memory_backend_interface->p_ops->element_hierarchy_member_read_memory_backend_interface
            == NULL) {
        msg_stderr(
                "element_hierarchy_member_get_memory_backend_interface isn't defined\n");
        return -(ENOENT);
...
int element_hierarchy_member_get_array_subrange0(
    ...
    if (p_memory_backend_interface->p_ops->element_hierarchy_member_get_array_subrange0
            == NULL) {
        msg_stderr(
                "element_hierarchy_member_get_array_subrange0 isn't defined\n");
        return -(ENOENT);
    }
...
int element_hierarchy_member_get_pointer_subrange0(
    ...
    if (p_memory_backend_interface->p_ops->element_hierarchy_member_get_pointer_subrange0
            == NULL) {
        msg_stderr(
                "element_hierarchy_member_get_pointer_subrange0 isn't defined\n");
        return -(ENOENT);
    }
...
int element_hierarchy_member_cast_pointer(
    ...
    if (p_memory_backend_interface->p_ops->element_hierarchy_member_cast_pointer
            == NULL) {
...
int memory_block_top_free_memory(struct memory_block_top *p_memory_block_top) {
    struct memory_backend_interface *p_memory_backend_interface;
    ...
    p_memory_backend_interface =
            p_memory_block_top->memory_block.p_memory_backend_interface;

    if (p_memory_backend_interface->p_ops->memory_block_top_free_memory
            == NULL) {
    ...
    for (i = 0; i < p_memory_block_top->memory_block_references_count; ++i) {
        if (p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element_hierarchy_member
                == NULL) {
            assert(
                    p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element->p_element_type->p_actual_type->id
                            == DW_TAG_pointer_type);

            if (p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element->pointer.p_casted_memory_backend_interface
                    != NULL) {
                memory_backend_interface_put(
                        &p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element->pointer.p_casted_memory_backend_interface);
            }
            if (p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element->pointer.p_original_memory_backend_interface
                    != NULL) {
                memory_backend_interface_put(
                        &p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element->pointer.p_original_memory_backend_interface);
            }
        } else {
            assert(
                    p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element_hierarchy_member->p_element_type_hierarchy_member->p_actual_type->id
                            == DW_TAG_pointer_type);

            if (p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element_hierarchy_member->pointer.p_casted_memory_backend_interface
                    != NULL) {
                memory_backend_interface_put(
                        &p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element_hierarchy_member->pointer.p_casted_memory_backend_interface);
            }
            if (p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element_hierarchy_member->pointer.p_original_memory_backend_interface
                    != NULL) {
                memory_backend_interface_put(
                        &p_memory_block_top->pp_memory_block_references[i]->reference_location.p_element_hierarchy_member->pointer.p_original_memory_backend_interface);
            }
        }
    }

    for (i = 0; i < p_memory_block_top->memory_blocks_count; ++i) {
        for (j = 0; j < p_memory_block_top->pp_memory_blocks[j]->elements_count;
                ++j) {
            if (p_memory_block_top->pp_memory_blocks[j]->p_elements->p_element_type->p_actual_type->id
                    == DW_TAG_array_type) {
                if (p_memory_block_top->pp_memory_blocks[j]->p_elements->array.p_memory_backend_interface
                        != NULL) {
                    memory_backend_interface_put(
                            &p_memory_block_top->pp_memory_blocks[j]->p_elements->array.p_memory_backend_interface);
                }
            }
        }
    }
    ...
    for (i = 0; i < p_memory_block_top->memory_blocks_count; ++i) {
        if (p_memory_block_top->pp_memory_blocks[j]->p_memory_backend_interface
                != NULL) {
            memory_backend_interface_put(
                    &p_memory_block_top->pp_memory_blocks[j]->p_memory_backend_interface);
        }
    }
...
int object_root_validate(struct object_root *p_object_root) {
    ...
    if (p_object_root->p_memory_backend_interface->p_ops->object_root_validate
            == NULL) { /*optional function*/
        return 0;
    }

    return p_object_root->p_memory_backend_interface->p_ops->object_root_validate(
            p_object_root);
}
