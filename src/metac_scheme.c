...
static int metac_scheme_clean(struct metac_scheme *p_metac_scheme);

/*****************************************************************************/

static int metac_scheme_delete(struct metac_scheme **pp_metac_scheme) {
...
static metac_refcounter_object_ops_t _metac_scheme_refcounter_object_ops = {
    .unknown_object_ops = { .metac_unknown_object_delete =
            _metac_scheme_unknown_object_delete, }, };

/*****************************************************************************/

struct metac_scheme* metac_scheme_get(struct metac_scheme *p_metac_scheme) {

if (p_metac_scheme != NULL
        && metac_refcounter_object_get(&p_metac_scheme->refcounter_object)
                != NULL) {
...
int metac_scheme_put(struct metac_scheme **pp_metac_scheme) {

if (pp_metac_scheme != NULL && (*pp_metac_scheme) != NULL
        && metac_refcounter_object_put(&(*pp_metac_scheme)->refcounter_object)
                == 0) {
...
static int generic_cast_type_delete(
    struct generic_cast_type **pp_generic_cast_type, metac_count_t types_count) {
...
static void discriminator_clean(struct discriminator *p_discriminator) {
...
static int discriminator_init(struct discriminator *p_discriminator,
    struct metac_type *p_root_type, char *global_path,
...
static int scheme_with_array_init(struct scheme_with_array *p_scheme_with_array,
    struct metac_type *p_root_type, char *global_path, char *value_path,
    metac_type_annotation_t *p_override_annotations,
...
if (p_actual_type->array_type_info.is_flexible != 0) {
    ...
    p_target_actual_type = metac_type_get_actual_type(
    ...
    if (p_target_actual_type != NULL
        ...
        p_scheme_with_array->array_elements_count.cb =
...
if (p_annotation != NULL) {
    if (p_actual_type->array_type_info.is_flexible != 0) {
        ...
        p_scheme_with_array->array_elements_count.p_data =
...
if (p_actual_type->pointer_type_info.type != NULL) {
    ...
    if (p_target_actual_type != NULL
        ...
        p_scheme_with_pointer->array_elements_count.cb =
                metac_array_elements_1d_with_null;
    }
}

if (p_annotation != NULL) {
    p_scheme_with_pointer->generic_cast.cb =
            p_annotation->value->generic_cast.cb;
    p_scheme_with_pointer->generic_cast.p_data =
    ...
    p_scheme_with_pointer->array_elements_count.cb =
            p_annotation->value->array_elements_count.cb;
    p_scheme_with_pointer->array_elements_count.p_data =
            p_annotation->value->array_elements_count.data;
...
if (p_scheme_with_pointer->generic_cast.p_types != NULL) {
    ...
    for (i = 0; i < p_scheme_with_pointer->generic_cast.types_count; ++i) {
        ...
        if (p_scheme_with_pointer->generic_cast.p_types[i].p_child_value_scheme
                != NULL) {

            metac_scheme_put(
                    &p_scheme_with_pointer->generic_cast.p_types[i].p_child_value_scheme);
        }
    }

    generic_cast_type_delete(&p_scheme_with_pointer->generic_cast.p_types,
            p_scheme_with_pointer->generic_cast.types_count);
}
...
p_scheme_with_hierarchy->members = (struct metac_scheme**) calloc(
        p_scheme_with_hierarchy->members_count,
...
struct metac_scheme *p_value_scheme = scheme_with_hierarchy_get_value_scheme(
        p_scheme_with_hierarchy);

if (discriminator_init(&p_scheme_with_hierarchy->union_discriminator,
        p_root_type, global_path, p_override_annotations,
        (metac_scheme_is_hierarchy_member_scheme(p_value_scheme) == 1) ?
...
if (p_scheme_with_hierarchy == NULL || p_value_scheme == NULL) {
...
struct metac_scheme *p_metac_scheme, struct metac_type *p_root_type,
char *global_path, char *value_path, char *hirarchy_path,
metac_type_annotation_t *p_override_annotations,
...
metac_count_t member_id, struct metac_type_member_info *p_member_info) {
struct metac_scheme *p_parent_metac_scheme;

if (p_metac_scheme == NULL || p_current_hierarchy == NULL) {
...
p_metac_scheme->hierarchy_member.precondition.expected_discriminator_value =
...
p_parent_metac_scheme = metac_hierarchy_member_scheme_get_parent_scheme(
    p_metac_scheme);
if (p_parent_metac_scheme != NULL) {

assert(metac_scheme_is_hierachy_scheme(p_parent_metac_scheme));

p_metac_scheme->hierarchy_member.offset +=
        p_parent_metac_scheme->hierarchy_member.offset;
...
case DW_TAG_structure_type:
case DW_TAG_union_type:
    res = scheme_with_hierarchy_init(&p_metac_scheme->hierarchy, p_root_type,
            global_path, value_path, p_override_annotations,
            p_metac_scheme->p_actual_type);
    break;
case DW_TAG_array_type:
    res = scheme_with_array_init(&p_metac_scheme->array, p_root_type,
            global_path, value_path, p_override_annotations,
            p_metac_scheme->p_actual_type);
    break;
case DW_TAG_pointer_type:
    res = scheme_with_pointer_init(&p_metac_scheme->pointer, p_root_type,
            global_path, value_path, p_override_annotations,
            p_metac_scheme->p_actual_type);
    break;
}
...
struct metac_type *p_root_type, char *global_path, char *value_path,
char *hierarchy_path, metac_type_annotation_t *p_override_annotations,
struct scheme_with_hierarchy *p_current_hierarchy,
struct discriminator *p_discriminator,
metac_discriminator_value_t expected_discriminator_value,
metac_count_t member_id, struct metac_type_member_info *p_member_info) {
...
if (metac_refcounter_object_init(&p_metac_scheme->refcounter_object,
    &_metac_scheme_refcounter_object_ops, NULL) != 0) {
...
if (metac_scheme_init_as_hierarchy_member_scheme(p_metac_scheme, p_root_type,
    global_path, value_path, hierarchy_path, p_override_annotations,
    p_current_hierarchy, p_discriminator, expected_discriminator_value,
    member_id, p_member_info) != 0) {
...
return metac_scheme_get(
    scheme_with_hierarchy_get_value_scheme(p_metac_scheme->p_current_hierarchy));
}

metac_flag_t metac_scheme_is_hierachy_scheme(
struct metac_scheme *p_metac_scheme) {

if (p_metac_scheme != NULL && p_metac_scheme->p_actual_type != NULL
...
metac_flag_t metac_scheme_is_union_scheme(struct metac_scheme *p_metac_scheme) {
...
metac_flag_t metac_scheme_is_base_scheme(struct metac_scheme *p_metac_scheme) {
...
metac_flag_t metac_scheme_is_array_scheme(struct metac_scheme *p_metac_scheme) {
...
metac_flag_t metac_scheme_is_pointer_scheme(struct metac_scheme *p_metac_scheme) {
...
char *global_path, struct metac_scheme *p_value_scheme);

/*****************************************************************************/

static void hierarchy_top_scheme_container_discriminator_clean(
struct hierarchy_top_scheme_container_discriminator *p_hierarchy_top_scheme_container_discriminator) {
...
msg_stderr(
        "Can't init p_element_type_hierarchy_top_scheme_container_discriminator with NULL p_discriminator \n");
return (-EINVAL);
}

p_hierarchy_top_scheme_container_discriminator->p_discriminator =
    p_discriminator;
...
struct hierarchy_top_scheme_container_discriminator **pp_hierarchy_top_scheme_container_discriminator) {

_delete_start_(hierarchy_top_scheme_container_discriminator);
hierarchy_top_scheme_container_discriminator_clean(
...
if (hierarchy_top_scheme_container_discriminator_init(
    p_hierarchy_top_scheme_container_discriminator, p_discriminator) != 0) {

hierarchy_top_scheme_container_discriminator_delete(
...
struct hierarchy_top_scheme_container_hierarchy_member *p_hierarchy_top_scheme_container_hierarchy_member,
struct metac_scheme *p_value_scheme) {
...
p_hierarchy_top_scheme_container_hierarchy_member->p_value_scheme =
    metac_scheme_get(p_value_scheme);
...
struct hierarchy_top_scheme_container_hierarchy_member **pp_hierarchy_top_scheme_container_hierarchy_member) {

_delete_start_(hierarchy_top_scheme_container_hierarchy_member);
hierarchy_top_scheme_container_hierarchy_member_clean(
...
if (hierarchy_top_scheme_container_hierarchy_member_init(
    p_hierarchy_top_scheme_container_hierarchy_member, p_metac_scheme) != 0) {

hierarchy_top_scheme_container_hierarchy_member_delete(
...
p_value_scheme_container_discriminator =
    hierarchy_top_scheme_container_discriminator_create(p_discriminator);
...
cds_list_add_tail(&p_value_scheme_container_discriminator->list,
...
p_value_scheme_container_hierarchy_member =
    hierarchy_top_scheme_container_hierarchy_member_create(p_value_scheme);
...
cds_list_add_tail(&p_value_scheme_container_hierarchy_member->list,
...
p_metac_scheme = scheme_with_hierarchy_get_value_scheme(
    p_scheme_with_hierarchy);
if (p_metac_scheme == NULL) {
...
p_scheme_with_hierarchy->members[indx] =
        metac_scheme_create_as_hierarchy_member_scheme(
                p_hierarchy_top_scheme_builder->p_root_type, member_global_path,
                member_value_path, member_hirarchy_path,
                p_hierarchy_top_scheme_builder->p_override_annotations,
                p_scheme_with_hierarchy, p_discriminator,
                expected_discriminator_value, indx, p_member_info);
if (p_scheme_with_hierarchy->members[indx] == NULL) {
    msg_stddbg(
            "global_path: %s failed because metac_scheme_create_as_hierarchy_member failed\n",
            global_path);
...
res = hierarchy_top_scheme_builder_schedule_hierarchy_member(
        p_hierarchy_top_scheme_builder, member_global_path,
        p_scheme_with_hierarchy->members[indx]);
if (res != 0) {
...
struct metac_type_member_info *p_member_info =
        &p_actual_type->structure_type_info.members[i];

if (hierarchy_top_scheme_builder_add_hierarchy_member(
        p_hierarchy_top_scheme_builder, p_scheme_with_hierarchy->members[i])
        != 0) {

    msg_stderr(
            "hierarchy_top_scheme_builder_add_hierarchy_member failed for for %s and %s\n",
            global_path, p_member_info->name);
...
if (hierarchy_top_scheme_builder_add_discriminator(
    p_hierarchy_top_scheme_builder, p_discriminator) != 0) {
...
p_scheme_with_hierarchy->members[indx] =
        metac_scheme_create_as_hierarchy_member_scheme(
                ...
                member_value_path, member_hirarchy_path,
                p_hierarchy_top_scheme_builder->p_override_annotations,
                p_scheme_with_hierarchy, p_discriminator, indx, indx,
                p_member_info);
if (p_scheme_with_hierarchy->members[indx] == NULL) {
...
if (hierarchy_top_scheme_builder_add_hierarchy_member(
        p_hierarchy_top_scheme_builder, p_scheme_with_hierarchy->members[indx])
        != 0) {
...
struct metac_type_member_info *p_member_info =
        &p_actual_type->union_type_info.members[i];
...
msg_stddbg("process global_path: %s, value_path: %s, hirarchy_path: %s\n",
    global_path, value_path, hirarchy_path);

switch (p_actual_type->id) {
case DW_TAG_structure_type:
res = hierarchy_top_scheme_builder_process_structure(
        p_hierarchy_top_scheme_builder, global_path, value_path, hirarchy_path,
        p_actual_type, p_scheme_with_hierarchy);
break;
case DW_TAG_union_type:
res = hierarchy_top_scheme_builder_process_union(p_hierarchy_top_scheme_builder,
        global_path, value_path, hirarchy_path, p_actual_type,
        p_scheme_with_hierarchy);
break;
default:
res = (-EINVAL);
...
if (hierarchy_top_scheme_builder_process_hierarchy(
        p_hierarchy_top_scheme_builder, global_path,
        p_value_scheme->path_within_value,
        p_value_scheme->hierarchy_member.path_within_hierarchy,
        p_value_scheme->p_actual_type, &p_value_scheme->hierarchy) != 0) {
...
struct scheduler *p_engine, struct scheduler_task *p_task, int error_flag) {
...
struct hierarchy_top_scheme_builder_hierarchy_member_task
* p_hierarchy_top_scheme_builder_hierarchy_member_task =
...
cds_list_add_tail(&p_task->list,
    &p_hierarchy_top_scheme_builder->executed_tasks);
...
struct hierarchy_top_scheme_builder_hierarchy_member_task **pp_hierarchy_top_scheme_builder_hierarchy_member_task) {
...
if ((*pp_hierarchy_top_scheme_builder_hierarchy_member_task)->p_value_scheme
    != NULL) {

metac_scheme_put(
        &(*pp_hierarchy_top_scheme_builder_hierarchy_member_task)->p_value_scheme);
}
...
scheduler_task_fn_t fn, char *global_path, struct metac_scheme *p_value_scheme) {
...
p_hierarchy_top_scheme_builder_hierarchy_member_task->global_path = strdup(
    global_path);
p_hierarchy_top_scheme_builder_hierarchy_member_task->p_value_scheme =
    metac_scheme_get(p_value_scheme);
...
hierarchy_top_scheme_builder_hierarchy_member_task_delete(
        &p_hierarchy_top_scheme_builder_hierarchy_member_task);
return NULL;
}
...
struct hierarchy_top_scheme_builder_hierarchy_member_task *p_task =
    hierarchy_top_scheme_builder_hierarchy_member_task_create(
            hierarchy_top_scheme_builder_process_hierarchy_member_fn,
            global_path, p_value_scheme);
...
if (scheduler_add_task_to_front(&p_hierarchy_top_scheme_builder->scheduler,
    &p_task->task) != 0) {
...
struct hierarchy_top_scheme_container_discriminator *p_discriminator,
    *_p_discriminator;
struct hierarchy_top_scheme_container_hierarchy_member *p_member, *_p_member;

cds_list_for_each_entry_safe(p_discriminator, _p_discriminator,
    &p_value_scheme_container->discriminator_list, list)
{
cds_list_del(&p_discriminator->list);
hierarchy_top_scheme_container_discriminator_delete(&p_discriminator);
}

cds_list_for_each_entry_safe(p_member, _p_member,
    &p_value_scheme_container->hierarchy_member_list, list)
{
...
struct hierarchy_top_scheme_builder *p_hierarchy_top_scheme_builder,
struct metac_type *p_root_type, metac_type_annotation_t *p_override_annotations) {
...
struct hierarchy_top_scheme_builder_hierarchy_member_task
    *p_hierarchy_member_task, *_p_hierarchy_member_task;

scheduler_clean(&p_hierarchy_top_scheme_builder->scheduler);

cds_list_for_each_entry_safe(p_hierarchy_member_task, _p_hierarchy_member_task,
    &p_hierarchy_top_scheme_builder->executed_tasks, task.list)
{

cds_list_del(&p_hierarchy_member_task->task.list);
hierarchy_top_scheme_builder_hierarchy_member_task_delete(
        &p_hierarchy_member_task);
}

hierarchy_top_scheme_container_clean(
    &p_hierarchy_top_scheme_builder->container);
...
/*get arrays lengths */
cds_list_for_each_entry(p_discriminator,
    &p_hierarchy_top_scheme_builder->container.discriminator_list, list)
{
++p_value_scheme->hierarchy_top.discriminators_count;
}
cds_list_for_each_entry(p_member,
    &p_hierarchy_top_scheme_builder->container.hierarchy_member_list, list)
{
...
p_value_scheme->hierarchy_top.pp_discriminators =
        (struct discriminator**) calloc(
                p_value_scheme->hierarchy_top.discriminators_count,
                sizeof(*p_value_scheme->hierarchy_top.pp_discriminators));
        ...
        &p_hierarchy_top_scheme_builder->container.discriminator_list, list)
{
    ...
    p_value_scheme->hierarchy_top.pp_discriminators[i] =
            p_discriminator->p_discriminator;
...
p_value_scheme->hierarchy_top.pp_members = (struct metac_scheme**) calloc(
        p_value_scheme->hierarchy_top.members_count,
        sizeof(*p_value_scheme->hierarchy_top.pp_members));
        ...
        &p_hierarchy_top_scheme_builder->container.hierarchy_member_list, list)
{
    ...
    assert(
            metac_scheme_is_hierarchy_member_scheme(p_member->p_value_scheme)
                    == 1);

    p_value_scheme->hierarchy_top.pp_members[i] = metac_scheme_get(
            p_member->p_value_scheme);
...
struct metac_scheme *p_metac_scheme, metac_count_t i) {
...
struct metac_scheme *p_metac_scheme, struct metac_type *p_root_type,
char *global_path, char *value_path,
metac_type_annotation_t *p_override_annotations) {
...
hierarchy_top_scheme_builder_init(&hierarchy_top_scheme_builder, p_root_type,
    p_override_annotations);

p_metac_scheme->p_current_hierarchy = NULL;

if (scheme_with_hierarchy_init(&p_metac_scheme->hierarchy, p_root_type,
    global_path, value_path, p_override_annotations,
...
if (hierarchy_top_scheme_builder_process_hierarchy(
    &hierarchy_top_scheme_builder, global_path, value_path, "",
    p_metac_scheme->p_actual_type, &p_metac_scheme->hierarchy) != 0) {
...
if (metac_scheme_finalize_as_hierarchy_top_scheme(&hierarchy_top_scheme_builder,
    p_metac_scheme) != 0) {
...
static int metac_scheme_init_as_indexable(struct metac_scheme *p_metac_scheme,
struct metac_type *p_root_type, char *global_path, char *value_path,
metac_type_annotation_t *p_override_annotations, struct metac_type *p_type) {
...
case DW_TAG_structure_type:
case DW_TAG_union_type:
res = metac_scheme_init_as_hierarchy_top_scheme(p_metac_scheme, p_root_type,
        global_path, value_path, p_override_annotations);
break;
case DW_TAG_array_type:
res = scheme_with_array_init(&p_metac_scheme->array, p_root_type, global_path,
        value_path, p_override_annotations, p_metac_scheme->p_actual_type);
break;
case DW_TAG_pointer_type:
res = scheme_with_pointer_init(&p_metac_scheme->pointer, p_root_type,
        global_path, value_path, p_override_annotations,
...
metac_flag_t metac_scheme_is_indexable(struct metac_scheme *p_metac_scheme) {
...
struct metac_type *p_type, char *global_path, char *value_path,
struct metac_type *p_root_type, metac_type_annotation_t *p_override_annotations) {
...
if (metac_scheme_init_as_indexable(p_metac_scheme, p_root_type, global_path,
    value_path, p_override_annotations, p_type) != 0) {
...
struct value_scheme_builder *p_value_scheme_builder, char *global_path,
struct metac_scheme *p_metac_scheme);
...
msg_stderr(
        "Can't init value_scheme_container_value_scheme with NULL p_metac_scheme\n");
return (-EINVAL);
}

p_value_scheme_container_value_scheme->p_metac_scheme = metac_scheme_get(
    p_metac_scheme);
...
struct value_scheme_container_value_scheme **pp_value_scheme_container_value_scheme) {

_delete_start_(value_scheme_container_value_scheme);
value_scheme_container_value_scheme_clean(
...
if (value_scheme_container_value_scheme_init(
    p_value_scheme_container_value_scheme, p_metac_scheme) != 0) {

value_scheme_container_value_scheme_delete(
...
p_value_scheme_container_value_scheme =
    value_scheme_container_value_scheme_create(p_metac_scheme);
if (p_value_scheme_container_value_scheme == NULL) {
...
cds_list_add_tail(&p_value_scheme_container_value_scheme->list,
...
struct value_scheme_builder *p_value_scheme_builder, char *global_path,
char *value_path, struct metac_scheme *p_metac_scheme) {
...
if (value_scheme_builder_add_array(p_value_scheme_builder, p_metac_scheme)
    != 0) {
...
p_metac_scheme->array.p_child_value_scheme = metac_scheme_create_as_indexable(
    p_metac_scheme->p_actual_type->array_type_info.type, target_global_path,
    target_value_path, p_value_scheme_builder->p_root_type,
...
} else if (value_scheme_builder_schedule_value_scheme(p_value_scheme_builder,
    target_global_path, p_metac_scheme->array.p_child_value_scheme) != 0) {
...
if (value_scheme_builder_add_pointer(p_value_scheme_builder, p_metac_scheme)
    != 0) {
...
assert(
        metac_scheme_is_hierarchy_member_scheme(
                p_metac_scheme->hierarchy_top.pp_members[i]) == 1);

if (p_metac_scheme->hierarchy_top.pp_members[i]->p_actual_type != NULL
        && (p_metac_scheme->hierarchy_top.pp_members[i]->p_actual_type->id
                == DW_TAG_array_type
                || p_metac_scheme->hierarchy_top.pp_members[i]->p_actual_type->id
                        == DW_TAG_pointer_type)) {
    ...
    case DW_TAG_array_type:
        ...
        if (value_scheme_builder_process_value_scheme_array(
                p_value_scheme_builder, target_global_path, target_value_path,
                p_metac_scheme->hierarchy_top.pp_members[i]) != 0) {

            msg_stderr(
                    "value_scheme_builder_process_value_scheme_array failed for %s\n",
                    target_global_path);
    ...
    case DW_TAG_pointer_type:
        ...
        if (value_scheme_builder_process_value_scheme_pointer(
            ...
            msg_stderr(
                    "value_scheme_builder_process_value_scheme_pointer failed for %s\n",
                    target_global_path);
...
switch (p_metac_scheme->p_actual_type->id) {
case DW_TAG_array_type:
res = value_scheme_builder_process_value_scheme_array(p_value_scheme_builder,
        global_path, p_metac_scheme->path_within_value, p_metac_scheme);
break;
case DW_TAG_pointer_type:
res = value_scheme_builder_process_value_scheme_pointer(p_value_scheme_builder,
        global_path, p_metac_scheme->path_within_value, p_metac_scheme);
break;
case DW_TAG_structure_type:
case DW_TAG_union_type:
res = value_scheme_builder_process_value_scheme_hierarchy_top(
        p_value_scheme_builder, global_path, p_metac_scheme->path_within_value,
        p_metac_scheme);
break;
}
...
struct value_scheme_builder_value_scheme_task
* p_value_scheme_builder_value_scheme_task =
...
return value_scheme_builder_process_value_scheme(p_value_scheme_builder,
...
struct value_scheme_builder_value_scheme_task **pp_value_scheme_builder_value_scheme_task) {
...
scheduler_task_fn_t fn, char *global_path, struct metac_scheme *p_metac_scheme) {
...
p_value_scheme_builder_value_scheme_task->p_metac_scheme = metac_scheme_get(
    p_metac_scheme);
...
value_scheme_builder_value_scheme_task_delete(
        &p_value_scheme_builder_value_scheme_task);
return NULL;
}
...
struct value_scheme_builder_value_scheme_task *p_task =
    value_scheme_builder_value_scheme_task_create(
            value_scheme_builder_process_value_scheme_fn, global_path,
            p_metac_scheme);
if (p_task == NULL) {
...
if (scheduler_add_task_to_front(&p_value_scheme_builder->scheduler,
    &p_task->task) != 0) {
...
cds_list_for_each_entry_safe(value_scheme, _value_scheme,
    &p_value_scheme_container->pointers, list)
{
...
cds_list_for_each_entry_safe(value_scheme, _value_scheme,
    &p_value_scheme_container->arrays, list)
{
...
cds_list_for_each_entry_safe(p_task, _p_task,
    &p_value_scheme_builder->executed_tasks, task.list)
{
...
static void value_scheme_clean(struct value_scheme *p_value_scheme) {
...
static int value_scheme_init(struct value_scheme *p_value_scheme,
struct value_scheme_builder *p_value_scheme_builder) {
...
/*get arrays lengths */
cds_list_for_each_entry(_value_scheme,
    &p_value_scheme_builder->container.arrays, list)
{
++p_value_scheme->arrays_value_schemes_count;
}
cds_list_for_each_entry(_value_scheme,
    &p_value_scheme_builder->container.pointers, list)
{
...
p_value_scheme->pp_arrays_value_schemes = (struct metac_scheme**) calloc(
        p_value_scheme->arrays_value_schemes_count,
        sizeof(*p_value_scheme->pp_arrays_value_schemes));
if (p_value_scheme->pp_arrays_value_schemes == NULL) {
        ...
        &p_value_scheme_builder->container.arrays, list)
{
    ...
    p_value_scheme->pp_arrays_value_schemes[i] = metac_scheme_get(
            _value_scheme->p_metac_scheme);
...
p_value_scheme->pp_pointers_value_schemes = (struct metac_scheme**) calloc(
        p_value_scheme->pointers_value_schemes_count,
        sizeof(*p_value_scheme->pp_pointers_value_schemes));
if (p_value_scheme->pp_pointers_value_schemes == NULL) {
        ...
        &p_value_scheme_builder->container.pointers, list)
{
    ...
    p_value_scheme->pp_pointers_value_schemes[i] = metac_scheme_get(
            _value_scheme->p_metac_scheme);
...
static int value_scheme_delete(struct value_scheme **pp_value_scheme) {
...
if (value_scheme_init(p_value_scheme, p_value_scheme_builder) != 0) {
...
metac_flag_t metac_scheme_is_value_scheme(struct metac_scheme *p_metac_scheme) {
...
value_scheme_builder_init(&value_scheme_builder, p_root_type,
    p_override_annotations);

if (metac_scheme_init_as_indexable(p_metac_scheme, p_root_type, global_path, "",
    p_override_annotations, p_type) != 0) {
...
if (value_scheme_builder_process_value_scheme(&value_scheme_builder,
    global_path, p_metac_scheme) != 0) {
...
if (value_scheme_builder_finalize(&value_scheme_builder, p_metac_scheme) != 0) {
...
struct metac_type *p_type, char *global_path, struct metac_type *p_root_type,
metac_type_annotation_t *p_override_annotations) {
...
if (metac_scheme_init_as_value_scheme(p_metac_scheme, p_root_type, global_path,
    p_override_annotations, p_type) != 0) {
...
char *global_path, struct metac_scheme *p_metac_scheme);
/*****************************************************************************/

static void deep_value_scheme_container_pointer_scheme_clean(
struct deep_value_scheme_container_pointer_scheme *p_deep_value_scheme_container_pointer_scheme) {
...
metac_scheme_put(
        &p_deep_value_scheme_container_pointer_scheme->p_pointer_metac_scheme);
}
}
...
p_deep_value_scheme_container_pointer_scheme->p_pointer_metac_scheme =
    metac_scheme_get(p_pointer_metac_scheme);
if (p_deep_value_scheme_container_pointer_scheme->p_pointer_metac_scheme
    == NULL) {

msg_stderr("metac_scheme_get failed\n");
deep_value_scheme_container_pointer_scheme_clean(
...
struct deep_value_scheme_container_pointer_scheme **pp_deep_value_scheme_container_pointer_scheme) {
...
deep_value_scheme_container_pointer_scheme_delete(
        &p_deep_value_scheme_container_pointer_scheme);
return NULL;
}
...
metac_scheme_put(
        &p_deep_value_scheme_container_value_scheme->p_object_metac_scheme);
}

cds_list_for_each_entry_safe(p_container_pointer_scheme,
    _p_container_pointer_scheme,
    &p_deep_value_scheme_container_value_scheme->pointers, list)
{
...
p_container_pointer_scheme = deep_value_scheme_container_pointer_scheme_create(
...
cds_list_add_tail(&p_container_pointer_scheme->list,
...
if (p_pointer_metac_scheme == NULL || p_object_metac_scheme == NULL) {
...
struct deep_value_scheme_container_value_scheme **pp_deep_value_scheme_container_value_scheme) {
...
if (deep_value_scheme_container_value_scheme_init(
    p_deep_value_scheme_container_value_scheme, p_pointer_metac_scheme,
    p_object_metac_scheme) != 0) {

deep_value_scheme_container_value_scheme_delete(
        &p_deep_value_scheme_container_value_scheme);
return NULL;
}
...
p_new_pointer_member_info =
        p_pointer_metac_scheme->hierarchy_member.p_member_info;
}

cds_list_for_each_entry(_value_scheme,
    &p_deep_value_scheme_builder->container.objects, list)
{
...
struct metac_scheme
* p_entry_pointer_metac_scheme = cds_list_first_entry(
        &_value_scheme->pointers, struct deep_value_scheme_container_pointer_scheme, list)->p_pointer_metac_scheme;
struct metac_type *p_entry_object_type =
        _value_scheme->p_object_metac_scheme->p_type;
...
if (metac_scheme_is_hierarchy_member_scheme(p_pointer_metac_scheme) == 1) {
    ...
    p_new_pointer_member_info =
            p_entry_pointer_metac_scheme->hierarchy_member.p_member_info;
}
...
p_deep_value_scheme_container_value_scheme =
    deep_value_scheme_container_value_scheme_create(p_pointer_metac_scheme,
            p_object_metac_scheme);
if (p_deep_value_scheme_container_value_scheme == NULL) {

msg_stddbg(
        "wasn't able to create p_deep_value_scheme_container_value_scheme\n");
return (-EFAULT);
}

cds_list_add_tail(&p_deep_value_scheme_container_value_scheme->list,
...
p_container_value_scheme = deep_value_scheme_builder_find_object(
    p_deep_value_scheme_builder, p_metac_scheme, p_type);

if (p_container_value_scheme != NULL) {

p_result = metac_scheme_get(p_container_value_scheme->p_object_metac_scheme);
...
if (deep_value_scheme_container_value_scheme_add_pointer(
        p_container_value_scheme, p_metac_scheme) != 0) {
...
p_result = metac_scheme_create_as_value_scheme(p_type, global_path,
...
if (deep_value_scheme_builder_add_value_scheme(p_deep_value_scheme_builder,
    p_metac_scheme, p_result) != 0) {
...
if (deep_value_scheme_builder_schedule_value_scheme(p_deep_value_scheme_builder,
    global_path, p_result) != 0) {
...
static int _filter_pointer(struct metac_type *p_metac_type) {
...
p_actual_target_metac_type = metac_type_get_actual_type(
        p_actual_metac_type->pointer_type_info.type);
}
...
p_metac_scheme->pointer.p_default_child_value_scheme =
        deep_value_scheme_builder_update_record_or_create_new_and_schedule(
                p_deep_value_scheme_builder, target_global_path, p_metac_scheme,
                p_metac_scheme->p_actual_type->pointer_type_info.type);

if (p_metac_scheme->pointer.p_default_child_value_scheme == NULL) {

    msg_stderr(
            "deep_value_scheme_builder_update_record_or_create_new_and_schedule failed for %s\n",
            global_path);
    free(target_global_path);
    return (-EFAULT);
}
...
target_global_path = alloc_sptrinf("generic_cast<*%s>(%s)[]",
...
p_metac_scheme->pointer.generic_cast.p_types[i].p_child_value_scheme =
        deep_value_scheme_builder_update_record_or_create_new_and_schedule(
                p_deep_value_scheme_builder, target_global_path, p_metac_scheme,
                p_metac_scheme->pointer.generic_cast.p_types[i].p_type);

if (p_metac_scheme->pointer.generic_cast.p_types[i].p_child_value_scheme
        == NULL) {

    msg_stderr(
            "deep_value_scheme_builder_schedule_value_scheme failed for %s generic_cast %d\n",
            global_path, i);
...
for (i = 0; i < p_metac_scheme->p_value_scheme->pointers_value_schemes_count;
    ++i) {
...
if (deep_value_scheme_builder_process_pointer(p_deep_value_scheme_builder,
        target_global_path,
        p_metac_scheme->p_value_scheme->pp_pointers_value_schemes[i]) != 0) {

    msg_stderr(
            "deep_value_scheme_builder_process_pointer failed for %s pointer %d\n",
            global_path, i);
...
struct deep_value_scheme_builder_value_scheme_task
* p_deep_value_scheme_builder_value_scheme_task =
...
struct deep_value_scheme_builder_value_scheme_task **pp_deep_value_scheme_builder_value_scheme_task) {
...
metac_scheme_put(
        &(*pp_deep_value_scheme_builder_value_scheme_task)->p_metac_scheme);
}
...
p_deep_value_scheme_builder_value_scheme_task->global_path = strdup(
    global_path);
p_deep_value_scheme_builder_value_scheme_task->p_metac_scheme =
    metac_scheme_get(p_metac_scheme);
...
msg_stderr("wasn't able to duplicate global_path or p_metac_scheme\n");
deep_value_scheme_builder_value_scheme_task_delete(
        &p_deep_value_scheme_builder_value_scheme_task);
return NULL;
}
...
struct deep_value_scheme_builder_value_scheme_task *p_task =
    deep_value_scheme_builder_value_scheme_task_create(
            deep_value_scheme_builder_process_value_scheme_fn, global_path,
            p_metac_scheme);
if (p_task == NULL) {
...
if (scheduler_add_task_to_front(&p_deep_value_scheme_builder->scheduler,
    &p_task->task) != 0) {
...
cds_list_for_each_entry_safe(value_scheme, _value_scheme,
    &p_deep_value_scheme_container->objects, list)
{
...
cds_list_for_each_entry_safe(p_task, _p_task,
    &p_deep_value_scheme_builder->executed_tasks, task.list)
{
...
for (i = 0; i < p_deep_value_scheme->value_schemes_count; ++i) {
    ...
    if (p_deep_value_scheme->p_value_schemes[i].pp_pointers != NULL) {
        ...
        for (j = 0; j < p_deep_value_scheme->p_value_schemes[i].pointers_count;
                ++j) {

            if (p_deep_value_scheme->p_value_schemes[i].pp_pointers[j]
                    != NULL) {

                deep_value_scheme_unprocess_pointer(
                        p_deep_value_scheme->p_value_schemes[i].pp_pointers[j]);
                metac_scheme_put(
                        &p_deep_value_scheme->p_value_schemes[i].pp_pointers[j]);
            }
        }
...
for (i = 0; i < p_deep_value_scheme->value_schemes_count; ++i) {
    ...
    if (p_deep_value_scheme->p_value_schemes[i].p_value_scheme != NULL) {
        metac_scheme_put(
                &p_deep_value_scheme->p_value_schemes[i].p_value_scheme);
    }
...
static int deep_value_scheme_init(struct deep_value_scheme *p_deep_value_scheme,
struct deep_value_scheme_builder *p_deep_value_scheme_builder) {
...
p_deep_value_scheme->p_value_schemes =
        (struct deep_value_scheme_value_scheme*) calloc(
                p_deep_value_scheme->value_schemes_count,
                sizeof(*p_deep_value_scheme->p_value_schemes));
if (p_deep_value_scheme->p_value_schemes == NULL) {
        ...
        &p_deep_value_scheme_builder->container.objects, list)
{
    ...
    p_deep_value_scheme->p_value_schemes[i].p_value_scheme = metac_scheme_get(
            _value_scheme->p_object_metac_scheme);
    if (p_deep_value_scheme->p_value_schemes[i].p_value_scheme == NULL) {
    ...
    if (p_deep_value_scheme->p_value_schemes[i].pointers_count > 0) {
        ...
        p_deep_value_scheme->p_value_schemes[i].pp_pointers =
                (struct metac_scheme**) calloc(
                        p_deep_value_scheme->p_value_schemes[i].pointers_count,
                        sizeof(*p_deep_value_scheme->p_value_schemes[i].pp_pointers));
        if (p_deep_value_scheme->p_value_schemes[i].pp_pointers == NULL) {
        ...
        {
            p_deep_value_scheme->p_value_schemes[i].pp_pointers[j] =
                    metac_scheme_get(_pointer_scheme->p_pointer_metac_scheme);
...
if (deep_value_scheme_init(p_deep_value_scheme, p_deep_value_scheme_builder)
    != 0) {
...
return deep_value_scheme_delete(
    &p_metac_scheme->p_value_scheme->p_deep_value_scheme);
}
...
p_metac_scheme->p_value_scheme->p_deep_value_scheme = deep_value_scheme_create(
    p_deep_value_scheme_builder);
...
deep_value_scheme_builder_init(&deep_value_scheme_builder, p_root_type,
    p_override_annotations);
...
if (deep_value_scheme_builder_process_value_scheme(&deep_value_scheme_builder,
    global_path, p_metac_scheme) != 0) {
...
if (deep_value_scheme_builder_finalize(&deep_value_scheme_builder,
    p_metac_scheme) != 0) {
...
if (metac_scheme_init_as_deep_value_scheme(p_metac_scheme, p_root_type,
    global_path, p_override_annotations, p_type) != 0) {
...
struct metac_scheme* metac_scheme_create(struct metac_type *p_type,
metac_type_annotation_t *p_override_annotations, metac_flag_t go_deep) {

if (go_deep == 0) {
return metac_scheme_create_as_value_scheme(p_type, "", p_type,
        p_override_annotations);
}

return metac_scheme_create_as_deep_value_scheme(p_type, "", p_type,
    p_override_annotations);
}
