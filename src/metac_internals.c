...
static int element_type_top_builder_schedule_element_type_from_array(
        ...
        char *global_path, struct metac_type *p_type,
...
static char* build_member_path(char *path, char *name) {
    ...
    member_path = calloc(sizeof(char),
            path_size + (size_t) 1 + name_size + (size_t) 1);
    if (member_path == NULL)
        return NULL;
...
/*****************************************************************************/
int discriminator_delete(struct discriminator **pp_discriminator) {
...
struct discriminator* discriminator_create(struct metac_type *p_root_type,
        char *global_path, metac_type_annotation_t *p_override_annotations,
    ...
    p_annotation = metac_type_annotation(p_root_type, global_path,
    ...
    if (p_previous_discriminator != NULL) { /*copy precondition*/
        p_discriminator->precondition.p_discriminator =
...
static int element_type_hierarchy_top_container_init_discriminator(
    ...
    if (p_discriminator == NULL) {
        msg_stderr(
                "Can't init p_element_type_hierarchy_top_container_discriminator with NULL p_discriminator \n");
        return (-EINVAL);
    }
    p_element_type_hierarchy_top_container_discriminator->p_discriminator =
            p_discriminator;
    return 0;
}
...
static struct element_type_hierarchy_top_container_discriminator* element_type_hierarchy_top_container_create_discriminator(
    ...
    msg_stddbg(
            "element_type_hierarchy_top_container_create_discriminator exit\n");
...
static int element_type_hierarchy_top_builder_add_discriminator(
    ...
    /* create */
    p_element_type_hierarchy_top_container_discriminator =
            element_type_hierarchy_top_container_create_discriminator(
                    p_discriminator);
    if (p_element_type_hierarchy_top_container_discriminator == NULL) {
        msg_stddbg(
                "wasn't able to create p_element_type_hierarchy_top_container_discriminator\n");
        return (-EFAULT);
    }
    /* add to list */
    cds_list_add_tail(
...
int element_type_array_init(struct metac_type *p_root_type, char *global_path,
        metac_type_annotation_t *p_override_annotations,
    ...
    if (p_actual_type->array_type_info.is_flexible != 0) {
        p_target_actual_type = metac_type_actual_type(
                p_actual_type->array_type_info.type);
        if (p_target_actual_type != NULL
                && p_target_actual_type->id == DW_TAG_base_type
                && (p_target_actual_type->base_type_info.encoding
                        == DW_ATE_signed_char
                        || p_target_actual_type->base_type_info.encoding
                                == DW_ATE_unsigned_char)
                && p_target_actual_type->name != NULL
                && (strcmp(p_target_actual_type->name, "char") == 0
                        || strcmp(p_target_actual_type->name, "unsigned char")
                                == 0)) {
            p_element_type_array->array_elements_count.cb =
                    metac_array_elements_1d_with_null;
        }
    }
    ...
    if (p_annotation != NULL) {
        if (p_actual_type->array_type_info.is_flexible != 0) {
            p_element_type_array->array_elements_count.cb =
                    p_annotation->value->array_elements_count.cb;
            p_element_type_array->array_elements_count.p_data =
                    p_annotation->value->array_elements_count.data;
        } else {
            if (p_annotation->value->array_elements_count.cb != NULL) {
                msg_stderr(
                        "Annotations for non-flexible array with global path %s defines array_elements_count that won't be used\n",
                        global_path);
            }
        }
        ...
        if (p_annotation->value->generic_cast.cb != NULL) {
            msg_stderr(
                    "Annotations for array with global path %s defines generic_cast that won't be used\n",
                    global_path);
        }
        if (p_annotation->value->discriminator.cb != NULL) {
            msg_stderr(
                    "Annotations for array with global path %s defines discriminator that won't be used\n",
                    global_path);
        }
    }
    return 0;
}
void element_type_array_clean(struct element_type_array *p_element_type_array) {
    if (p_element_type_array->annotation_key != NULL) {
...
static struct generic_cast_type* generic_cast_type_create(
        struct metac_type **types, metac_count_t *p_types_count) {
    metac_count_t i = 0;
    struct generic_cast_type *p_generic_cast_type;

    if (types == NULL || p_types_count == NULL)
        return NULL;
...
int element_type_pointer_init(struct metac_type *p_root_type, char *global_path,
        metac_type_annotation_t *p_override_annotations,
        struct metac_type *p_actual_type,
    ...
    p_element_type_pointer->array_elements_count.cb =
            metac_array_elements_single;
    ...
    if (p_annotation != NULL) {
        p_element_type_pointer->generic_cast.cb =
                p_annotation->value->generic_cast.cb;
        p_element_type_pointer->generic_cast.p_data =
                p_annotation->value->generic_cast.data;
        ...
        p_element_type_pointer->array_elements_count.cb =
                p_annotation->value->array_elements_count.cb;
        p_element_type_pointer->array_elements_count.p_data =
                p_annotation->value->array_elements_count.data;

        if (p_annotation->value->discriminator.cb != NULL) {
            msg_stderr(
                    "Annotations for pointer with global path %s defines discriminator that won't be used\n",
                    global_path);
        }
    }
    return 0;
...
int element_type_hierarchy_init(struct metac_type *p_root_type,
        char *global_path, metac_type_annotation_t *p_override_annotations,
        struct metac_type *p_actual_type,
    ...
    const metac_type_annotation_t *p_annotation;
    assert(
            p_actual_type->id == DW_TAG_structure_type
    ...
    p_element_type_hierarchy->members_count =
            p_actual_type->structure_type_info.members_count;
...
if ((*pp_element_type_hierarchy_member)->p_actual_type)
    ...
    case DW_TAG_union_type:
        element_type_hierarchy_clean(
                &(*pp_element_type_hierarchy_member)->hierarchy);
        break;
    case DW_TAG_array_type:
        element_type_array_clean(&(*pp_element_type_hierarchy_member)->array);
        break;
    case DW_TAG_pointer_type:
        element_type_pointer_clean(
                &(*pp_element_type_hierarchy_member)->pointer);
        break;
    }
...
return element_type_hierarchy_get_element_hierarchy_member(
        p_element_type_hierarchy_member->p_hierarchy);
}
...
struct element_type_hierarchy_member* element_type_hierarchy_member_create(
    struct metac_type *p_root_type, char *global_path, char *hirarchy_path,
    metac_type_annotation_t *p_override_annotations, metac_count_t member_id,
    struct discriminator *p_discriminator,
...
p_parent_element_type_hierarchy_member =
        element_type_hierarchy_member_get_parent_element_hierarchy_member(
                p_element_type_hierarchy_member);
if (p_parent_element_type_hierarchy_member != NULL) {
    p_element_type_hierarchy_member->offset +=
            p_parent_element_type_hierarchy_member->offset;
}

p_element_type_hierarchy_member->p_type = p_member_info->type;
if (p_element_type_hierarchy_member->p_type != NULL) {
    p_element_type_hierarchy_member->p_actual_type = metac_type_actual_type(
            p_member_info->type);
    p_element_type_hierarchy_member->byte_size = metac_type_byte_size(
            p_member_info->type);
}
...
if (p_element_type_hierarchy_member->p_actual_type != NULL) {
    if (p_element_type_hierarchy_member->p_actual_type->id
            == DW_TAG_base_type) {
        p_element_type_hierarchy_member->base_type.p_bit_offset =
                p_member_info->p_bit_offset;
        p_element_type_hierarchy_member->base_type.p_bit_size =
                p_member_info->p_bit_size;
    } else {
        int res;
        if (p_member_info->p_bit_offset != NULL
                || p_member_info->p_bit_size != NULL) {
        ...
        case DW_TAG_structure_type:
        case DW_TAG_union_type:
            res = element_type_hierarchy_init(p_root_type, global_path,
                    p_override_annotations,
                    p_element_type_hierarchy_member->p_actual_type,
                    &p_element_type_hierarchy_member->hierarchy, p_hierarchy);
            break;
        case DW_TAG_array_type:
            res = element_type_array_init(p_root_type, global_path,
                    p_override_annotations,
                    p_element_type_hierarchy_member->p_actual_type,
                    &p_element_type_hierarchy_member->array);
            break;
        case DW_TAG_pointer_type:
            res = element_type_pointer_init(p_root_type, global_path,
                    p_override_annotations,
                    p_element_type_hierarchy_member->p_actual_type,
                    &p_element_type_hierarchy_member->pointer);
            break;
        }

        if (res != 0) {
            msg_stddbg("initialization failed\n");
            element_type_hierarchy_member_delete(
                    &p_element_type_hierarchy_member);
            return NULL;
        }
    }
...
if (p_element_type_hierarchy_member == NULL) {
    msg_stderr(
            "Can't init p_element_type_hierarchy_top_container_element_type_hierarchy_member with NULL p_element_type_hierarchy_member \n");
    return (-EINVAL);
}
p_element_type_hierarchy_top_container_element_type_hierarchy_member->p_element_type_hierarchy_member =
        p_element_type_hierarchy_member;
return 0;
}
...
/* create */
p_element_type_hierarchy_top_container_element_type_hierarchy_member =
        element_type_hierarchy_top_container_create_element_type_hierarchy_member(
                p_element_type_hierarchy_member);
if (p_element_type_hierarchy_top_container_element_type_hierarchy_member
        == NULL) {
    msg_stddbg(
            "wasn't able to create p_element_type_hierarchy_top_container_element_type_hierarchy_member\n");
    return (-EFAULT);
}
...
static int element_type_hierarchy_top_builder_process_structure(
    struct element_type_hierarchy_top_builder *p_element_type_hierarchy_top_builder,
    char *global_path, char *hirarchy_path,
    struct discriminator *p_discriminator,
    metac_discriminator_value_t expected_discriminator_value,
    struct metac_type *p_actual_type,
    struct element_type_hierarchy *p_hierarchy) {
...
for (i = p_actual_type->structure_type_info.members_count; i > 0; i--) {
    int indx = i - 1;
    struct metac_type_member_info *p_member_info =
            &p_actual_type->structure_type_info.members[indx];
    int res;
    ...
    member_hirarchy_path = build_member_path(hirarchy_path,
            p_member_info->name);
    ...
    p_hierarchy->members[indx] = element_type_hierarchy_member_create(
            ...
            member_global_path, member_hirarchy_path,
            p_element_type_hierarchy_top_builder->p_override_annotations, indx,
            p_discriminator, expected_discriminator_value, p_hierarchy,
            p_member_info);
    if (p_hierarchy->members[indx] == NULL) {
        msg_stddbg(
                "global_path: %s failed because element_type_hierarchy_member_create failed\n",
                global_path);
    ...
    res =
            element_type_hierarchy_top_builder_schedule_element_type_hierarchy_member(
                    p_element_type_hierarchy_top_builder, member_global_path,
                    p_hierarchy->members[indx], p_hierarchy);
    if (res != 0) {
...
p_discriminator = discriminator_create(
        p_element_type_hierarchy_top_builder->p_root_type, global_path,
        p_element_type_hierarchy_top_builder->p_override_annotations,
        p_previous_discriminator, previous_expected_discriminator_value,
        p_hierarchy);
if (p_discriminator == NULL) {
    msg_stddbg("global_path: %s failed to create discriminator\n", global_path);
    return (-EFAULT);
}
if (element_type_hierarchy_top_builder_add_discriminator(
        p_element_type_hierarchy_top_builder, p_discriminator) != 0) {
...
for (i = p_actual_type->union_type_info.members_count; i > 0; i--) {
    int indx = i - 1;
    struct metac_type_member_info *p_member_info =
            &p_actual_type->union_type_info.members[indx];
    int res;
    char *member_global_path;
    char *member_hirarchy_path;
    ...
    p_hierarchy->members[indx] = element_type_hierarchy_member_create(
            ...
            member_global_path, member_hirarchy_path,
            p_element_type_hierarchy_top_builder->p_override_annotations, indx,
            p_discriminator, indx, p_hierarchy, p_member_info);
    if (p_hierarchy->members[indx] == NULL) {
        msg_stddbg(
                "global_path: %s failed because element_type_hierarchy_member_create failed\n",
                global_path);
...
case DW_TAG_structure_type:
    res = element_type_hierarchy_top_builder_process_structure(
            p_element_type_hierarchy_top_builder, global_path, hirarchy_path,
            p_discriminator, expected_discriminator_value, p_actual_type,
            p_hierarchy);
    break;
case DW_TAG_union_type:
...
default:
    res = (-EINVAL);
    msg_stderr(
            "incorrect type %d element_type_hierarchy_top_builder_process_hierarchy\n",
            p_actual_type->id);
    assert(0);
    break;
}
...
if (p_element_type_hierarchy_member->p_actual_type
    ...
    if (element_type_hierarchy_top_builder_process_hierarchy(
            p_element_type_hierarchy_top_builder, global_path,
            p_element_type_hierarchy_member->path_within_hierarchy,
            p_element_type_hierarchy_member->precondition.p_discriminator,
            p_element_type_hierarchy_member->precondition.expected_discriminator_value,
            p_element_type_hierarchy_member->p_actual_type,
            &p_element_type_hierarchy_member->hierarchy, p_parent_hierarchy)
            != 0) {
...
static int element_type_hierarchy_top_builder_delete_hierarchy_member_task(
    struct element_type_hierarchy_top_builder_element_type_hierarchy_member_task **pp_element_type_hierarchy_top_builder_element_type_hierarchy_member_task) {
_delete_start_(
        element_type_hierarchy_top_builder_element_type_hierarchy_member_task);
if ((*pp_element_type_hierarchy_top_builder_element_type_hierarchy_member_task)->global_path) {
    free(
            (*pp_element_type_hierarchy_top_builder_element_type_hierarchy_member_task)->global_path);
    (*pp_element_type_hierarchy_top_builder_element_type_hierarchy_member_task)->global_path =
            NULL;
}
_delete_finish(
...
static int element_type_hierarchy_top_builder_process_element_type_hierarchy_member_fn(
    struct traversing_engine *p_engine, struct traversing_engine_task *p_task,
    int error_flag) {
struct element_type_hierarchy_top_builder
...
struct element_type_hierarchy_top_builder_element_type_hierarchy_member_task
* p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task =
cds_list_entry(p_task, struct element_type_hierarchy_top_builder_element_type_hierarchy_member_task, task);
cds_list_add_tail(&p_task->list,
        &p_element_type_hierarchy_top_builder->hierarchy_executed_tasks);
...
static struct element_type_hierarchy_top_builder_element_type_hierarchy_member_task* element_type_hierarchy_top_builder_schedule_element_type_hierarchy_member_with_fn(
    struct element_type_hierarchy_top_builder *p_element_type_hierarchy_top_builder,
    traversing_engine_task_fn_t fn, char *global_path,
    struct element_type_hierarchy_member *p_element_type_hierarchy_member,
    struct element_type_hierarchy *p_parent_hierarchy) {
_create_(element_type_hierarchy_top_builder_element_type_hierarchy_member_task);

p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task->task.fn =
        fn;
p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task->global_path =
        strdup(global_path);
p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task->p_element_type_hierarchy_member =
...
p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task->p_parent_hierarchy =
        p_parent_hierarchy;
if (p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task->global_path
        == NULL) {
    msg_stderr("wasn't able to duplicate global_path\n");
    element_type_hierarchy_top_builder_delete_hierarchy_member_task(
            &p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task);
    return NULL;
}
if (add_traversing_task_to_front(
        &p_element_type_hierarchy_top_builder->hierarchy_traversing_engine,
        &p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task->task)
        != 0) {
...
return (element_type_hierarchy_top_builder_schedule_element_type_hierarchy_member_with_fn(
        ...
        global_path, p_element_type_hierarchy_member, p_parent_hierarchy)
        != NULL) ? 0 : (-EFAULT);

}
static int element_type_hierarchy_top_container_init(
    struct element_type_hierarchy_top_container *p_element_type_hierarchy_top_container) {
CDS_INIT_LIST_HEAD(
        &p_element_type_hierarchy_top_container->discriminator_type_list);
CDS_INIT_LIST_HEAD(
        &p_element_type_hierarchy_top_container->element_type_hierarchy_member_type_list);
return 0;
}
static void element_type_hierarchy_top_container_clean(
    struct element_type_hierarchy_top_container *p_element_type_hierarchy_top_container,
    metac_flag_t keep_data) {
struct element_type_hierarchy_top_container_discriminator *_discriminator,
        *__discriminator;
struct element_type_hierarchy_top_container_element_type_hierarchy_member
        *_member, *__member;
cds_list_for_each_entry_safe(_member, __member,
        &p_element_type_hierarchy_top_container->element_type_hierarchy_member_type_list,
        list)
{
    cds_list_del(&_member->list);
    element_type_hierarchy_top_container_clean_element_type_hierarchy_member(
            _member, keep_data);
    free(_member);
}
cds_list_for_each_entry_safe(_discriminator, __discriminator,
        &p_element_type_hierarchy_top_container->discriminator_type_list, list)
{
    cds_list_del(&_discriminator->list);
    element_type_hierarchy_top_container_clean_discriminator(_discriminator,
            keep_data);
    free(_discriminator);
}
}
...
p_element_type_hierarchy_top_builder->p_override_annotations =
        p_override_annotations;
element_type_hierarchy_top_container_init(
        &p_element_type_hierarchy_top_builder->container);
CDS_INIT_LIST_HEAD(
        &p_element_type_hierarchy_top_builder->hierarchy_executed_tasks);
traversing_engine_init(
        &p_element_type_hierarchy_top_builder->hierarchy_traversing_engine);
return 0;
}
...
traversing_engine_clean(
        &p_element_type_hierarchy_top_builder->hierarchy_traversing_engine);
cds_list_for_each_entry_safe(p_task, _p_task,
        &p_element_type_hierarchy_top_builder->hierarchy_executed_tasks, list)
{
    struct element_type_hierarchy_top_builder_element_type_hierarchy_member_task
    * p_element_type_hierarchy_top_builder_element_type_hierarchy_member_task = cds_list_entry(
            p_task,
...
element_type_hierarchy_top_container_clean(
        &p_element_type_hierarchy_top_builder->container, keep_data);
...
if (p_element_type_hierarchy_top->members_count > 0) {
    p_element_type_hierarchy_top->pp_members =
            (struct element_type_hierarchy_member**) calloc(
                    p_element_type_hierarchy_top->members_count,
                    sizeof(*p_element_type_hierarchy_top->pp_members));
    ...
    cds_list_for_each_entry_safe(_member, __member,
            ...
            list)
    {
        p_element_type_hierarchy_top->pp_members[i] =
                _member->p_element_type_hierarchy_member;
...
if (p_element_type_hierarchy_top->discriminators_count > 0) {
    p_element_type_hierarchy_top->pp_discriminators =
            (struct discriminator**) calloc(
                    p_element_type_hierarchy_top->discriminators_count,
                    sizeof(*p_element_type_hierarchy_top->pp_discriminators));
    ...
    cds_list_for_each_entry_safe(_discriminator, __discriminator,
            ...
            list)
    {
        p_element_type_hierarchy_top->pp_discriminators[i] =
...
int element_type_hierarchy_top_init(struct metac_type *p_root_type,
    char *global_path, metac_type_annotation_t *p_override_annotations,
    struct metac_type *p_actual_type,
    struct element_type_hierarchy_top *p_element_type_hierarchy_top) {
struct element_type_hierarchy_top_builder element_type_hierarchy_top_builder;

element_type_hierarchy_top_builder_init(&element_type_hierarchy_top_builder,
        p_root_type, p_override_annotations);

if (element_type_hierarchy_init(p_root_type, global_path,
        p_override_annotations, p_actual_type,
        &p_element_type_hierarchy_top->hierarchy, NULL) != 0) {
    msg_stderr("element_type_hierarchy_init failed\n");
    element_type_hierarchy_top_builder_clean(
            &element_type_hierarchy_top_builder, 0);
    return (-EFAULT);
}

if (element_type_hierarchy_top_builder_process_hierarchy(
        &element_type_hierarchy_top_builder, global_path, "", NULL, 0,
        p_actual_type, &p_element_type_hierarchy_top->hierarchy, NULL) != 0) {
...
if (traversing_engine_run(
        &element_type_hierarchy_top_builder.hierarchy_traversing_engine) != 0) {
...
if (element_type_hierarchy_top_builder_finalize(
        &element_type_hierarchy_top_builder, p_element_type_hierarchy_top)
        != 0) {
...
element_type_hierarchy_top_builder_clean(&element_type_hierarchy_top_builder,
        1);
return 0;
}
/*****************************************************************************/
...
case DW_TAG_union_type: {
    ...
    for (i = 0; i < p_element_type->hierarchy_top.members_count; ++i) {
        if (p_element_type->hierarchy_top.pp_members[i]->p_actual_type != NULL
                && p_element_type->hierarchy_top.pp_members[i]->p_actual_type->id
                        == DW_TAG_array_type
                && p_element_type->hierarchy_top.pp_members[i]->p_actual_type->array_type_info.is_flexible) {
            if (p_element_type->hierarchy_top.pp_members[i]->offset
                    == p_element_type->byte_size)
                return 1;
            else {
                msg_stderr(
                        "flexible array isn't at the end of the structure\n");
                return 1;
            }
        }
    }
...
int element_type_delete(struct element_type **pp_element_type) {
...
struct element_type* element_type_create(struct metac_type *p_root_type,
    char *global_path, metac_type_annotation_t *p_override_annotations,
    struct metac_type *p_type) {
int res = 0;
...
case DW_TAG_structure_type:
case DW_TAG_union_type:
    res = element_type_hierarchy_top_init(p_root_type, global_path,
            p_override_annotations, p_element_type->p_actual_type,
            &p_element_type->hierarchy_top);
    break;
case DW_TAG_array_type:
    res = element_type_array_init(p_root_type, global_path,
            p_override_annotations, p_element_type->p_actual_type,
            &p_element_type->array);
    break;
case DW_TAG_pointer_type:
    res = element_type_pointer_init(p_root_type, global_path,
            p_override_annotations, p_element_type->p_actual_type,
            &p_element_type->pointer);
    break;
}
if (res != 0) {
    msg_stddbg("initialization failed\n");
...
p_element_type->is_potentially_flexible = element_type_is_potentially_flexible(
        p_element_type);
return p_element_type;
}
metac_flag_t element_type_is_hierachy(struct element_type *p_element_type) {
if (p_element_type != NULL && p_element_type->p_actual_type != NULL
        && (p_element_type->p_actual_type->id == DW_TAG_structure_type
                || p_element_type->p_actual_type->id == DW_TAG_union_type))
    return 1;
return 0;
}
metac_flag_t element_type_is_array(struct element_type *p_element_type) {
if (p_element_type != NULL && p_element_type->p_actual_type != NULL
        && (p_element_type->p_actual_type->id == DW_TAG_array_type))
    return 1;
return 0;
}
metac_flag_t element_type_is_pointer(struct element_type *p_element_type) {
if (p_element_type != NULL && p_element_type->p_actual_type != NULL
...
if (p_element_type == NULL) {
    msg_stderr(
            "Can't init precomple_type_container_element_type with NULL p_element_type \n");
    return (-EINVAL);
}
...
cds_list_for_each_entry_safe(_element_type, __element_type,
        &p_element_type_top_container->pointers_element_type_list, list)
{
...
cds_list_for_each_entry_safe(_element_type, __element_type,
        &p_element_type_top_container->arrays_element_type_list, list)
{
    cds_list_del(&_element_type->list);
    element_type_top_container_clean_element_type(_element_type, keep_data);
    free(_element_type);
...
static int element_type_top_builder_process_element_type_array(
    struct element_type_top_builder *p_element_type_top_builder,
    char *global_path, struct element_type *p_element_type,
    struct element_type_hierarchy_member *p_element_type_hierarchy_member) {
int i = 0;
int res = (-EINVAL);
struct metac_type *p_actual_type;
...
if (p_actual_type->array_type_info.type != NULL) {
    ...
    res = element_type_top_builder_schedule_element_type_from_array(
            p_element_type_top_builder, target_global_path,
            p_actual_type->array_type_info.type, p_member_info,
            &p_array->p_element_type);
    free(target_global_path);
    if (res != 0) {
        msg_stderr(
                "Can't element_type_top_builder_schedule_element_type_from_array for %s\n",
                global_path);
        return (-EFAULT);
    }
}
...
if (p_actual_type->pointer_type_info.type != NULL) {
    ...
    res = element_type_top_builder_schedule_element_type_from_pointer(
            p_element_type_top_builder, target_global_path,
            p_actual_type->pointer_type_info.type, p_member_info,
            &p_pointer->p_element_type);
    free(target_global_path);
    if (res != 0) {
        msg_stderr(
                "Can't element_type_top_builder_schedule_element_type_from_pointer for %s\n",
                global_path);
        return (-EFAULT);
    }
}
...
for (i = 0; i < p_pointer->generic_cast.types_count; ++i) {
    char *target_global_path = alloc_sptrinf("generic_cast<*%s>(%s)[]",
            p_pointer->generic_cast.p_types[i].p_type->name, global_path);
    if (target_global_path == NULL) {
    ...
    if (res != 0) {
        msg_stderr(
                "Can't element_type_top_builder_schedule_element_type_from_pointer for %s generic_cast %d\n",
                global_path, i);
        return (-EFAULT);
    }
}
...
for (i = 0; i < p_element_type->hierarchy_top.members_count; ++i) {
    ...
    if (p_element_type->hierarchy_top.pp_members[i]->p_actual_type
        ...
        case DW_TAG_array_type:
            ...
            if (res != 0) {
                msg_stderr(
                        "Can't element_type_top_builder_schedule_element_type_from_array for %s member %d\n",
                        global_path, i);
                free(target_global_path);
                return (-EFAULT);
            }
        ...
        case DW_TAG_pointer_type:
            ...
            if (res != 0) {
                msg_stderr(
                        "Can't element_type_top_builder_process_element_type_pointer for %s member %d\n",
                        global_path, i);
                free(target_global_path);
                return (-EFAULT);
            }
...
res = element_type_top_container_init_element_type(
        ...
        element_type_create(p_element_type_top_builder->p_root_type,
                global_path, p_element_type_top_builder->p_override_annotations,
                p_type), p_from_member_info);
if (res != 0) {
    msg_stddbg(
            "element_type_top_container_element_type_init returned error - deleting\n");
...
case DW_TAG_array_type:
    res = element_type_top_builder_process_element_type_array(
            p_element_type_top_builder, global_path,
            p_element_type_top_container_element_type->p_element_type, NULL);
    break;
case DW_TAG_pointer_type:
    res = element_type_top_builder_process_element_type_pointer(
...
/* create */
p_element_type_top_container_element_type =
        element_type_top_builder_element_type_create(p_element_type_top_builder,
                global_path, p_type, p_from_member_info);
if (p_element_type_top_container_element_type == NULL) {
    msg_stddbg(
            "wasn't able to create p_element_type_top_container_element_type\n");
    return (-EFAULT);
}
...
cds_list_add_tail(&p_element_type_top_container_element_type->list,
...
cds_list_for_each_entry(p_element_type_top_container_element_type,
        &p_element_type_top_builder->container.pointers_element_type_list, list)
{
    ...
    if (p_element_type_top_container_element_type->p_element_type->p_type
            == p_type
            && p_element_type_top_container_element_type->p_from_member_info
                    == p_from_member_info) {
...
struct element_type_top_builder_element_type_task
* p_element_type_top_builder_element_type_task =
cds_list_entry(p_task, struct element_type_top_builder_element_type_task, task);
cds_list_add_tail(&p_task->list,
        &p_element_type_top_builder->element_type_executed_tasks);
if (error_flag != 0)
    return (-EALREADY);
...
static int element_type_top_builder_delete_element_type_task(
    struct element_type_top_builder_element_type_task **pp_element_type_top_builder_element_type_task) {
...
if (p_element_type_top_builder_element_type_task->global_path == NULL) {
    msg_stderr("wasn't able to duplicate global_path\n");
    element_type_top_builder_delete_element_type_task(
            &p_element_type_top_builder_element_type_task);
    return NULL;
}
...
return (element_type_top_builder_schedule_element_type_with_fn(
        ...
        global_path, p_type, p_from_member_info, pp_element_type_to_store)
        != NULL) ? 0 : (-EFAULT);
}
static int element_type_top_builder_schedule_element_type_from_pointer(
    struct element_type_top_builder *p_element_type_top_builder,
...
traversing_engine_clean(
        &p_element_type_top_builder->element_type_traversing_engine);
cds_list_for_each_entry_safe(p_task, _p_task,
        &p_element_type_top_builder->element_type_executed_tasks, list)
{
    struct element_type_top_builder_element_type_task
    * p_element_type_top_builder_element_type_task = cds_list_entry(
            p_task,
...
element_type_top_container_clean(&p_element_type_top_builder->container,
        keep_data);
p_element_type_top_builder->p_root_type = NULL;
}
/*****************************************************************************/
void element_type_top_clean(struct element_type_top *p_element_type_top) {
metac_count_t i;
...
/*get arrays lengths */
cds_list_for_each_entry(_element_type,
        &p_element_type_top_builder->container.pointers_element_type_list, list)
{
    ++p_element_type_top->element_types_count;
}
cds_list_for_each_entry(_element_type,
        &p_element_type_top_builder->container.arrays_element_type_list, list)
{
    ++p_element_type_top->element_types_count;
}
...
if (p_element_type_top->element_types_count > 0) {
    p_element_type_top->pp_element_types = (struct element_type**) calloc(
            p_element_type_top->element_types_count,
            sizeof(*p_element_type_top->pp_element_types));
    if (p_element_type_top->pp_element_types == NULL) {
...
int element_type_top_init(struct metac_type *p_root_type, char *global_path,
    metac_type_annotation_t *p_override_annotations,
    struct element_type_top *p_element_type_top) {
struct element_type_top_builder element_type_top_builder;
element_type_top_builder_init(&element_type_top_builder, p_root_type,
        p_override_annotations);
...
p_element_type_top->p_element_type_for_pointer = element_type_create(
        p_root_type, global_path, p_override_annotations,
        p_element_type_top->p_pointer_type);
...
if (element_type_top_builder_process_element_type_pointer(
        &element_type_top_builder, global_path,
        p_element_type_top->p_element_type_for_pointer, NULL) != 0) {
...
if (traversing_engine_run(
        &element_type_top_builder.element_type_traversing_engine) != 0) {
...
/*fill in */
if (element_type_top_builder_finalize(&element_type_top_builder,
...
metac_precompiled_type_t* metac_precompile_type(struct metac_type *p_root_type,
    metac_type_annotation_t *p_override_annotations) {
_create_(metac_precompiled_type);

if (element_type_top_init(p_root_type, "ptr", p_override_annotations,
...
static int object_root_builder_schedule_object_root_container_memory_block_top(
    struct object_root_builder *p_object_root_builder, char *global_path,
    struct object_root_container_memory_block_top *p_object_root_container_memory_block_top);
/*****************************************************************************/
static int element_array_check_parents(struct element_array *p_element_array,
...
p_current_element =
        p_containing_element->p_memory_block->local_parent.p_element;
p_current_element_hierarchy_member =
        p_containing_element->p_memory_block->local_parent.p_element_hierarchy_member;

while (p_current_element != NULL) {
    if (p_current_element_hierarchy_member != NULL) {
        assert(
                p_current_element->p_element_type->p_actual_type->id
                        == DW_TAG_structure_type
                        || p_current_element->p_element_type->p_actual_type->id
                                == DW_TAG_union_type);
        assert(
                p_current_element_hierarchy_member->p_element_type_hierarchy_member->p_actual_type->id
                        == DW_TAG_array_type);
        assert(p_current_element_hierarchy_member->array.p_memory_block);
        assert(
                p_current_element_hierarchy_member->array.p_memory_block->p_array_info);
        if (metac_array_info_get_element_count(
                p_current_element_hierarchy_member->array.p_memory_block->p_array_info)
                > 1) {
            return -EFAULT;
        }
        ...
        assert(
                p_current_element->p_element_type->p_actual_type->id
                        == DW_TAG_array_type);
        assert(p_current_element->array.p_memory_block);
        assert(p_current_element->array.p_memory_block->p_array_info);
        if (metac_array_info_get_element_count(
                p_current_element->array.p_memory_block->p_array_info) > 1) {
            return -EFAULT;
        }
    }
    p_current_element =
            p_current_element->p_memory_block->local_parent.p_element;
    p_current_element_hierarchy_member =
            p_current_element->p_memory_block->local_parent.p_element_hierarchy_member;
}
return 0;
}
static void element_array_clean(struct element_array *p_element_array,
    struct element_type_array *p_element_type_array) {
if (p_element_array->p_memory_backend_interface) {
    memory_backend_interface_put(&p_element_array->p_memory_backend_interface);
}
}
static void element_pointer_clean(struct element_pointer *p_element_pointer,
    struct element_type_pointer *p_element_type_pointer) {
if (p_element_pointer->p_casted_memory_backend_interface != NULL) {
    memory_backend_interface_put(
            &p_element_pointer->p_casted_memory_backend_interface);
}
if (p_element_pointer->p_original_memory_backend_interface != NULL) {
    memory_backend_interface_put(
            &p_element_pointer->p_original_memory_backend_interface);
}
}
...
if (element_type_hierarchy_member_is_array(
        p_element_hierarchy_member->p_element_type_hierarchy_member) == 0) {
...
if (p_element_hierarchy_member->p_element_type_hierarchy_member->p_actual_type->array_type_info.is_flexible
        != 0) {

    if (element_hierarchy_member_get_array_subrange0(p_element_hierarchy_member)
            != 0) {
    ...
    metac_type_array_subrange_count(
            ...
            0, &p_element_hierarchy_member->array.subrange0_count);
}

if (p_element_hierarchy_member->array.is_flexible != 0
        && p_element_hierarchy_member->array.subrange0_count > 0
        && element_array_check_parents(&p_element_hierarchy_member->array,
                p_element_hierarchy_member->p_element,
                p_element_hierarchy_member) != 0) {
    msg_stderr(
            "flexible array got subrange0_count >0 and will cause fields overlap: %s\n",
            path_within_memory_block_top);
    return (-EFAULT);
}
...
assert(p_element_hierarchy_member);
if (element_type_hierarchy_member_is_pointer(
        p_element_hierarchy_member->p_element_type_hierarchy_member) == 0) {
...
if (element_hierarchy_member_read_memory_backend_interface(
        p_element_hierarchy_member,
        &p_element_hierarchy_member->pointer.p_original_memory_backend_interface)
        != 0) {
...
if (p_element_hierarchy_member->pointer.p_original_memory_backend_interface
        == NULL) {
    return 0;
}
...
/* set subrange0_count*/
if (element_hierarchy_member_get_pointer_subrange0(p_element_hierarchy_member)
        != 0) {
    msg_stderr("wasn't able to get subrange0 for pointer: %s\n",
            path_within_memory_block_top);
    return (-EFAULT);
...
static int element_hierarchy_member_init(
    struct element_hierarchy_member *p_element_hierarchy_member,
    char *path_within_memory_block_top, metac_count_t id,
    struct element_type_hierarchy_member *p_element_type_hierarchy_member,
...
switch (p_element_hierarchy_member->p_element_type_hierarchy_member->p_actual_type->id) {
case DW_TAG_pointer_type:
    if (element_hierarchy_member_init_pointer(p_element_hierarchy_member,
    ...
    break;
case DW_TAG_array_type:
    if (element_hierarchy_member_init_array(p_element_hierarchy_member,
            path_within_memory_block_top) != 0) {
...
if (element_hierarchy_member_init(p_element_hierarchy_member,
        path_within_memory_block_top, id, p_element_type_hierarchy_member,
        p_element) != 0) {
...
static void element_clean_hierarchy_top(struct element *p_element) {
metac_count_t i;
...
if (p_element_hierarchy_top->pp_discriminator_values) {
    for (i = 0; i < p_element_type_hierarchy_top->discriminators_count; ++i) {
        if (p_element_hierarchy_top->pp_discriminator_values[i] != NULL) {
            discriminator_value_delete(
                    &p_element_hierarchy_top->pp_discriminator_values[i]);
        }
    }
...
static int element_init_hierarchy_top(struct element *p_element,
    char *path_within_memory_block_top) {
metac_count_t i;
struct element_hierarchy_top *p_element_hierarchy_top;
struct element_type_hierarchy_top *p_element_type_hierarchy_top;
...
if (p_element_type_hierarchy_top->discriminators_count > 0) {
    p_element_hierarchy_top->pp_discriminator_values =
            (struct discriminator_value**) calloc(
                    p_element_type_hierarchy_top->discriminators_count,
                    sizeof(*p_element_hierarchy_top->pp_discriminator_values));
...
if (p_element_type_hierarchy_top->members_count > 0) {
    ...
    p_element_hierarchy_top->pp_members =
            (struct element_hierarchy_member**) calloc(
                    p_element_type_hierarchy_top->members_count,
                    sizeof(*p_element_hierarchy_top->pp_members));
    ...
    for (i = 0;
            i < p_element->p_element_type->hierarchy_top.discriminators_count;
            ++i) {
        metac_flag_t allocate = 0;

        assert(
                p_element->p_element_type->hierarchy_top.pp_discriminators[i]
                        != NULL);
        /*assumption: if we initialize all discriminators in order of their appearance - we won't need to resolve dependencides*/
        assert(
                p_element->p_element_type->hierarchy_top.pp_discriminators[i]->precondition.p_discriminator
                        == NULL
                        || p_element->p_element_type->hierarchy_top.pp_discriminators[i]->precondition.p_discriminator->id
                                < i);
        ...
        if (allocate == 0
            ...
            if (p_value != NULL
                    && p_value->value
                            == p_element->p_element_type->hierarchy_top.pp_discriminators[i]->precondition.expected_discriminator_value) {
                allocate = 1;
            }
        }

        if (allocate != 0) {
            p_element->hierarchy_top.pp_discriminator_values[i] =
                    discriminator_value_create(0);
            if (p_element->hierarchy_top.pp_discriminator_values[i] == NULL) {
                msg_stderr(
                        "can't allocated pp_discriminator_values %d for path %s\n",
    ...
    if (element_calculate_hierarchy_top_discriminator_values(p_element) != 0) {
        msg_stderr(
                "element_calculate_hierarchy_top_discriminator_values failed for path %s\n",
                path_within_memory_block_top);
        element_clean_hierarchy_top(p_element);
        return (-EFAULT);
    }
...
for (i = 0; i < p_element->p_element_type->hierarchy_top.members_count; ++i) {
    ...
    if (allocate == 0
            && p_element->p_element_type->hierarchy_top.pp_members[i]->precondition.p_discriminator
                    == NULL) {
        allocate = 1;
    }
    ...
    if (allocate == 0
        ...
        if (p_value != NULL
                && p_value->value
                        == p_element->p_element_type->hierarchy_top.pp_members[i]->precondition.expected_discriminator_value) {
            allocate = 1;
        }
    }

    if (allocate != 0) {
        p_element->hierarchy_top.pp_members[i] =
                element_hierarchy_member_create(path_within_memory_block_top, i,
                        p_element->p_element_type->hierarchy_top.pp_members[i],
                        p_element);
        if (p_element->hierarchy_top.pp_members[i] == NULL) {
            msg_stderr(
                    "element_hierarchy_member_create failed for member %d for path %s\n",
                    (int )i, path_within_memory_block_top);
            element_clean_hierarchy_top(p_element);
            return (-EFAULT);
        }
...
static void element_clean_array(struct element *p_element) {
struct element_array *p_element_array;
struct element_type_array *p_element_type_array;
...
static int element_init_array(struct element *p_element,
    char *path_within_memory_block_top) {

/* set pointer */
if (element_get_memory_backend_interface(p_element,
...
/* set is_flexible and subrange0_count*/
if (p_element->p_element_type->p_actual_type->array_type_info.is_flexible
        != 0) {
    ...
    metac_type_array_subrange_count(p_element->p_element_type->p_actual_type, 0,
            &p_element->array.subrange0_count);
}

if (p_element->array.is_flexible != 0 && p_element->array.subrange0_count > 0
        && element_array_check_parents(&p_element->array, p_element, NULL)
                != 0) {
    msg_stderr(
            "flexible array got subrange0_count >0 and will cause fields overlap: %s\n",
            path_within_memory_block_top);
...
static void element_clean_pointer(struct element *p_element) {
...
static int element_init_pointer(struct element *p_element,
    char *path_within_memory_block_top) {

/* set original pointer */
if (element_read_memory_backend_interface(p_element,
...
void element_clean(struct element *p_element) {
...
int element_init(struct element *p_element, char *path_within_memory_block_top,
    metac_count_t id, struct element_type *p_element_type,
    struct memory_block *p_memory_block) {
...
if (p_element->p_element_type != NULL) {
    ...
    case DW_TAG_structure_type:
    case DW_TAG_union_type:
        if (element_init_hierarchy_top(p_element, path_within_memory_block_top)
                != 0) {
        ...
        break;
    case DW_TAG_pointer_type:
        if (element_init_pointer(p_element, path_within_memory_block_top)
                != 0) {
        ...
        break;
    case DW_TAG_array_type:
        if (element_init_array(p_element, path_within_memory_block_top) != 0) {
...
void memory_block_clean(struct memory_block *p_memory_block) {
...
if (p_memory_block->p_array_info != NULL) {
    metac_array_info_delete(&p_memory_block->p_array_info);
}
}
int memory_block_init(struct memory_block *p_memory_block,
    ...
    struct element_type *p_element_type, metac_array_info_t *p_array_info,
    metac_flag_t is_flexible) {
metac_count_t i;
char *new_path_pattern;

if (p_local_parent_element != NULL) {
    p_memory_block->p_parent_memory_block =
            p_local_parent_element->p_memory_block;
    p_memory_block->parent_memory_block_offset = p_local_parent_element->id
            * p_local_parent_element->p_element_type->byte_size;
    /*TODO: check boundaries (byte_size)*/
    if (p_local_parent_element_hierarchy_member != NULL) {
        p_memory_block->parent_memory_block_offset +=
                p_local_parent_element_hierarchy_member->p_element_type_hierarchy_member->offset;
    }
}
...
if (memory_backend_interface_get(p_memory_backend_interface,
        &p_memory_block->p_memory_backend_interface) != 0) {
    msg_stderr("memory_backend_interface_get failed\n");
    memory_block_clean(p_memory_block);
    return -EFAULT;
}
p_memory_block->p_array_info = p_array_info;
p_memory_block->elements_count = metac_array_info_get_element_count(
        p_memory_block->p_array_info);
p_memory_block->byte_size = p_element_type->byte_size
        * metac_array_info_get_element_count(p_array_info);
p_memory_block->is_flexible = is_flexible;
p_memory_block->local_parent.p_element = p_local_parent_element;
p_memory_block->local_parent.p_element_hierarchy_member =
        p_local_parent_element_hierarchy_member;

p_memory_block->p_elements = calloc(p_memory_block->elements_count,
        sizeof(*p_memory_block->p_elements));
if (p_memory_block->p_elements == NULL) {
...
for (i = 0; i < p_memory_block->elements_count; ++i) {
    ...
    if (element_init(&p_memory_block->p_elements[i],
            new_path_within_memory_block_top, i, p_element_type, p_memory_block)
            != 0) {
...
int memory_block_delete(struct memory_block **pp_memory_block) {
_delete_start_(memory_block);
memory_block_clean(*pp_memory_block);
_delete_finish(memory_block);
return 0;
}
struct memory_block* memory_block_create(struct element *p_local_parent_element,
    struct element_hierarchy_member *p_local_parent_element_hierarchy_member,
    struct memory_backend_interface *p_memory_backend_interface,
    struct element_type *p_element_type, metac_array_info_t *p_array_info,
    metac_flag_t is_flexible) {
_create_(memory_block);
if (memory_block_init(p_memory_block, p_local_parent_element,
        p_local_parent_element_hierarchy_member, p_memory_backend_interface,
        p_element_type, p_array_info, is_flexible) != 0) {
...
/*****************************************************************************/
int memory_pointer_delete(struct memory_pointer **pp_memory_pointer) {
_delete_(memory_pointer);
return 0;
}
struct memory_pointer* memory_pointer_create(struct element *p_element,
    struct element_hierarchy_member *p_element_hierarchy_member) {
...
static int memory_block_top_builder_delete_memory_block_task(
    struct memory_block_top_builder_memory_block_task **pp_memory_block_top_builder_memory_block_task) {
...
if (keep_data == 0) {
    if ((*pp_memory_block_top_container_pointer)->p_memory_pointer != NULL) {
        memory_pointer_delete(
                &((*pp_memory_block_top_container_pointer)->p_memory_pointer));
    }
}
...
static struct memory_block_top_container_pointer* memory_block_top_container_pointer_create(
    char *global_path, struct memory_pointer *p_memory_pointer) {
if (global_path == NULL) {
...
if (p_memory_block_top_container_pointer->global_path == NULL) {
    ...
    memory_block_top_container_pointer_delete(
            &p_memory_block_top_container_pointer, 0);
    return NULL;
}
...
if (keep_data == 0
    ...
    memory_block_delete(
            &p_memory_block_top_container_memory_block->p_memory_block);
}
}
...
static int memory_block_top_container_memory_block_delete(
    struct memory_block_top_container_memory_block **pp_memory_block_top_container_memory_block,
    metac_flag_t keep_data) {
_delete_start_(memory_block_top_container_memory_block);
memory_block_top_container_memory_block_clean(
        *pp_memory_block_top_container_memory_block, keep_data);
...
static struct memory_block_top_container_memory_block* memory_block_top_container_memory_block_create(
    ...
    struct element_hierarchy_member *p_local_parent_element_hierarchy_member,
    struct memory_backend_interface *p_memory_backend_interface,
    metac_count_t byte_size, struct element_type *p_element_type,
    metac_array_info_t *p_array_info, metac_flag_t is_flexible) {
...
if (memory_block_top_container_memory_block_init(
        ...
        memory_block_create(p_local_parent_element,
                p_local_parent_element_hierarchy_member,
                p_memory_backend_interface, p_element_type, p_array_info,
                is_flexible)) != 0) {
    memory_block_top_container_memory_block_delete(
            &p_memory_block_top_container_memory_block, 0);
...
static void memory_block_top_container_clean(
    struct memory_block_top_container *p_memory_block_top_container,
    metac_flag_t keep_data) {
struct memory_block_top_container_memory_block *_container_memory_block,
        *__container_memory_block;
struct memory_block_top_container_pointer *_container_pointer,
        *__container_pointer;
cds_list_for_each_entry_safe(_container_memory_block, __container_memory_block,
        &p_memory_block_top_container->container_memory_block_list, list)
{
    cds_list_del(&_container_memory_block->list);
    memory_block_top_container_memory_block_delete(&_container_memory_block,
            keep_data);
}
cds_list_for_each_entry_safe(_container_pointer, __container_pointer,
        &p_memory_block_top_container->container_pointer_list, list)
{
...
CDS_INIT_LIST_HEAD(
        &p_memory_block_top_builder->memory_block_top_executed_tasks);
traversing_engine_init(
...
traversing_engine_clean(
        &p_memory_block_top_builder->memory_block_top_traversing_engine);
cds_list_for_each_entry_safe(p_task, _p_task,
        &p_memory_block_top_builder->memory_block_top_executed_tasks, list)
{
    struct memory_block_top_builder_memory_block_task
    * p_memory_block_top_builder_memory_block_task = cds_list_entry(
            p_task,
            struct memory_block_top_builder_memory_block_task, task);
    cds_list_del(&p_task->list);
    memory_block_top_builder_delete_memory_block_task(
            &p_memory_block_top_builder_memory_block_task);
}
memory_block_top_container_clean(&p_memory_block_top_builder->container,
        keep_data);
...
if (p_element_hierarchy_member == NULL) {
    ...
    assert(
            p_element_hierarchy_member->p_element_type_hierarchy_member->p_actual_type->id
                    == DW_TAG_array_type);
...
msg_stddbg("processing array for %s%s%s\n",
        ...
        (p_element == NULL && p_element_hierarchy_member == NULL) ?
                "" :
                p_element_hierarchy_member->p_element_type_hierarchy_member->path_within_hierarchy);

p_element_type = p_element_type_array->p_element_type;
p_array_info = metac_array_info_create_from_type(p_actual_type,
        p_element->pointer.subrange0_count);
if (p_array_info == NULL) {
    msg_stderr("metac_array_info_create_from_type failed\n");
    return (-EFAULT);
}

byte_size = p_element_type->byte_size
        * metac_array_info_get_element_count(p_array_info);
...
p_memory_block_top_container_memory_block =
        memory_block_top_container_memory_block_create(p_element,
                p_element_hierarchy_member,
                p_element_array->p_memory_backend_interface, byte_size,
                p_element_type, p_array_info, p_element_array->is_flexible);
if (p_memory_block_top_container_memory_block == NULL) {
    msg_stderr(
            "memory_block_top_container_memory_block_create failed for %s%s%s\n",
            (p_element == NULL) ? "" : p_element->path_within_memory_block_top,
            (p_element == NULL && p_element_hierarchy_member == NULL) ?
                    "" : ".",
            (p_element == NULL && p_element_hierarchy_member == NULL) ?
...
if (memory_block_top_builder_schedule_memory_block(p_memory_block_top_builder,
        p_memory_block_top_container_memory_block->p_memory_block) != 0) {
    msg_stderr(
            "memory_block_top_builder_schedule_memory_block_top_container_memory_block failed for %s%s%s\n",
            (p_element == NULL) ? "" : p_element->path_within_memory_block_top,
            (p_element == NULL && p_element_hierarchy_member == NULL) ?
                    "" : ".",
            (p_element == NULL && p_element_hierarchy_member == NULL) ?
...
cds_list_add_tail(&p_memory_block_top_container_memory_block->list,
...
msg_stderr(
        "PROCESSING block from %s%s%s: ptr=%p byte_size=%d type=%s elements_count=%d \n",
        (p_memory_block->local_parent.p_element == NULL) ?
                "top" :
                p_memory_block->local_parent.p_element->path_within_memory_block_top,
        (p_memory_block->local_parent.p_element == NULL
                && p_memory_block->local_parent.p_element_hierarchy_member
                        == NULL) ? "" : ".",
        (p_memory_block->local_parent.p_element == NULL
                && p_memory_block->local_parent.p_element_hierarchy_member
                        == NULL) ?
                "" :
                p_memory_block->local_parent.p_element_hierarchy_member->p_element_type_hierarchy_member->path_within_hierarchy,
        /*TODO: print something*/p_memory_block->p_memory_backend_interface,
        (int )p_memory_block->byte_size,
        p_memory_block->p_elements ?
                p_memory_block->p_elements[0].p_element_type->p_type->name :
                "<invalid>", (int )p_memory_block->elements_count);

for (i = 0; i < p_memory_block->elements_count; ++i) {
    struct memory_block_top_container_pointer *p_memory_block_top_container_pointer;
    struct metac_type *p_actual_type = metac_type_actual_type(
            p_memory_block->p_elements[i].p_element_type->p_type);
    ...
    switch (p_actual_type->id) {
    case DW_TAG_pointer_type:
        p_memory_block_top_container_pointer =
                memory_block_top_container_pointer_create(
        ...
        if (p_memory_block_top_container_pointer == NULL) {
            msg_stderr(
                    "wasn't able to created p_memory_block_top_container_pointer for %d, path %s\n",
                    (int )i,
                    p_memory_block->p_elements[i].path_within_memory_block_top);
            return (-EFAULT);
        }
        cds_list_add(&p_memory_block_top_container_pointer->list,
                &p_memory_block_top_builder->container.container_pointer_list);
        break;
    case DW_TAG_array_type:
        if (memory_block_top_builder_process_array(p_memory_block_top_builder,
                &p_memory_block->p_elements[i], NULL) != 0) {
            msg_stderr(
                    "object_root_builder_process_array failed for %d, path %s\n",
                    (int )i,
                    p_memory_block->p_elements[i].path_within_memory_block_top);
            return (-EFAULT);
        }
    ...
    case DW_TAG_union_type:
        for (j = 0;
                j
                        < p_memory_block->p_elements[i].p_element_type->hierarchy_top.members_count;
                ++j) {
            if (p_memory_block->p_elements[i].hierarchy_top.pp_members[j]->p_element_type_hierarchy_member->p_type->id
                    == DW_TAG_pointer_type) {
                p_memory_block_top_container_pointer =
                        memory_block_top_container_pointer_create(
                                p_memory_block_top_builder->global_path,
                                memory_pointer_create(
                                        &p_memory_block->p_elements[i],
                                        p_memory_block->p_elements[i].hierarchy_top.pp_members[j]));
                if (p_memory_block_top_container_pointer == NULL) {
                    msg_stderr(
                            "wasn't able to created p_memory_block_top_container_pointer for %d, path %s.%s\n",
                            (int )i,
                            p_memory_block->p_elements[i].path_within_memory_block_top,
                ...
                cds_list_add(&p_memory_block_top_container_pointer->list,
                        &p_memory_block_top_builder->container.container_pointer_list);
            } else if (p_memory_block->p_elements[i].hierarchy_top.pp_members[j]->p_element_type_hierarchy_member->p_type->id
                    == DW_TAG_array_type) {
                if (memory_block_top_builder_process_array(
                        ...
                        != 0) {
                    msg_stderr(
                            "object_root_builder_process_array failed for %d, path %s.%s\n",
                            (int )i,
                            p_memory_block->p_elements[i].path_within_memory_block_top,
                            p_memory_block->p_elements[i].hierarchy_top.pp_members[j]->p_element_type_hierarchy_member->path_within_hierarchy);
                    return (-EFAULT);
...
return memory_block_top_builder_process_memory_block(p_memory_block_top_builder,
...
static struct memory_block_top_builder_memory_block_task* memory_block_top_builder_schedule_memory_block_with_fn(
    struct memory_block_top_builder *p_memory_block_top_builder,
    traversing_engine_task_fn_t fn, struct memory_block *p_memory_block) {
...
return (memory_block_top_builder_schedule_memory_block_with_fn(
        p_memory_block_top_builder,
        memory_block_top_builder_process_memory_block_task_fn, p_memory_block)
        != NULL) ? 0 : (-EFAULT);
}
...
if (p_memory_block_top->memory_blocks_count > 0) {
    p_memory_block_top->pp_memory_blocks = (struct memory_block**) calloc(
            p_memory_block_top->memory_blocks_count,
            sizeof(*p_memory_block_top->pp_memory_blocks));
    ...
    ++i;
    cds_list_for_each_entry_safe(_container_memory_block,
            __container_memory_block,
            &p_memory_block_top_builder->container.container_memory_block_list,
            list)
    {
        struct memory_block *p_memory_block =
                _container_memory_block->p_memory_block;
        ...
        if (p_memory_block->local_parent.p_element != NULL) {
            ...
            p_memory_block->p_parent_memory_block =
                    p_memory_block->local_parent.p_element->p_memory_block;
            p_memory_block->parent_memory_block_offset =
                    p_memory_block->local_parent.p_element->id
                            * p_memory_block->local_parent.p_element->p_element_type->byte_size;
            if (p_memory_block->local_parent.p_element_hierarchy_member
                    != NULL) {
                p_memory_block->parent_memory_block_offset +=
                        p_memory_block->local_parent.p_element_hierarchy_member->p_element_type_hierarchy_member->offset;
            }
        }
        ...
        if (p_memory_block->p_parent_memory_block != NULL) {
            p_memory_block->p_memory_block_top =
                    p_memory_block->p_parent_memory_block->p_memory_block_top;
            p_memory_block->memory_block_top_offset =
                    p_memory_block->parent_memory_block_offset
                            + p_memory_block->p_parent_memory_block->parent_memory_block_offset;
        }
...
for (i = 0; i < p_memory_block_top->memory_blocks_count; ++i) {
    if (p_memory_block_top->pp_memory_blocks[i]->is_flexible) {
        ...
        p_memory_block_top->flexible_byte_size +=
                p_memory_block_top->pp_memory_blocks[i]->byte_size;
    }
}
...
cds_list_for_each_entry_safe(_container_pointer, __container_pointer,
        &p_memory_block_top_builder->container.container_pointer_list, list)
{
...
if (p_memory_block_top->memory_pointers_count > 0) {
    p_memory_block_top->pp_memory_pointers = (struct memory_pointer**) calloc(
            p_memory_block_top->memory_pointers_count,
            sizeof(*p_memory_block_top->pp_memory_pointers));
    ...
    cds_list_for_each_entry_safe(_container_pointer, __container_pointer,
            &p_memory_block_top_builder->container.container_pointer_list, list)
    {
        p_memory_block_top->pp_memory_pointers[i] =
                _container_pointer->p_memory_pointer;
        ++i;
    }
}
return 0;
...
/*****************************************************************************/
void memory_block_top_clean(struct memory_block_top *p_memory_block_top) {
metac_count_t i;
...
int memory_block_top_init(struct memory_block_top *p_memory_block_top,
    char *global_path,
    struct memory_backend_interface *p_memory_backend_interface,
    struct element_type *p_element_type, metac_array_info_t *p_array_info) {
struct memory_block_top_builder memory_block_top_builder;
msg_stderr("PROCESSING_TOP %s: ptr=%p type=%s elements_count=%d \n",
        global_path,
...
if (memory_block_top_builder_init(&memory_block_top_builder, global_path)
        != 0) {
    msg_stderr("memory_block_top_builder_init failed for %s\n", global_path);
    return (-EFAULT);
}
if (memory_block_init(&p_memory_block_top->memory_block, NULL, NULL,
        p_memory_backend_interface, p_element_type, p_array_info, 0/*TODO:*/)
        != 0) {
    msg_stderr(
            "tasks memory_block_top_container_memory_block_create finished with fail\n");
    memory_block_top_builder_clean(&memory_block_top_builder, 0);
    return (-EFAULT);
}
/* process main p_object_root_container_memory_block */
if (memory_block_top_builder_process_memory_block(&memory_block_top_builder,
        &p_memory_block_top->memory_block) != 0) {
    msg_stderr(
            "tasks object_root_container_memory_block_builder_process_object_root_container_memory_block finished with fail\n");
    memory_block_top_builder_clean(&memory_block_top_builder, 0);
...
if (traversing_engine_run(
        &memory_block_top_builder.memory_block_top_traversing_engine) != 0) {
    msg_stderr("tasks execution finished with fail\n");
    memory_block_top_builder_clean(&memory_block_top_builder, 0);
    memory_block_top_clean(p_memory_block_top);
    return (-EFAULT);
...
if (memory_block_top_builder_finalize(&memory_block_top_builder,
...
int memory_block_top_delete(struct memory_block_top **pp_memory_block_top) {
_delete_start_(memory_block_top);
memory_block_top_clean(*pp_memory_block_top);
_delete_finish(memory_block_top);
return 0;
}
struct memory_block_top* memory_block_top_create(char *global_path,
    struct memory_backend_interface *p_memory_backend_interface,
    struct element_type *p_element_type, metac_array_info_t *p_array_info) {
_create_(memory_block_top);
if (memory_block_top_init(p_memory_block_top, global_path,
        p_memory_backend_interface, p_element_type, p_array_info) != 0) {
...
if (p_memory_block_top == NULL || p_member_element_type == NULL
        || pp_element == NULL || pp_element_hierarchy_member == NULL
        || p_remaining_offset == NULL) {
    msg_stderr(
            "one of the pointers is NULL (invalid value for this function)\n");
    return -(EINVAL);
}
...
if (member_offset + p_member_element_type->byte_size
        > p_memory_block->byte_size) {
    msg_stderr(
            "member offset and byte_size are bigger than size of the memory_block_top\n");
    return -(EINVAL);
}
...
do {
    ...
    i = remaining_offset
            / p_memory_block->p_elements[0].p_element_type->byte_size;
    ...
    case DW_TAG_structure_type:
        ...
        for (j = 0;
            ...
            if (p_memory_block->p_elements[i].hierarchy_top.pp_members[j]
                    != NULL) {
                metac_data_member_location_t offset =
                        p_memory_block->p_elements[i].hierarchy_top.pp_members[j]->p_element_type_hierarchy_member->offset;
                ...
                if (offset == remaining_offset
                    ...
                    msg_stderr("found precise element %d member %d\n", (int )i,
                            (int )j);
                    *pp_element = &p_memory_block->p_elements[i];
                    *pp_element_hierarchy_member =
                            p_memory_block->p_elements[i].hierarchy_top.pp_members[j];
                    *p_remaining_offset = 0;
                    return 0;
                }

                if (p_memory_block->p_elements[i].p_element_type->p_actual_type->id
                        == DW_TAG_array_type) {
                    msg_stderr("going deeper into the next member array\n");
                    p_memory_block =
                            p_memory_block->p_elements[i].array.p_memory_block;
                    need_continue = 1;
                    break;
                }
                ...
                if (p_member_element_type->p_actual_type
                        != p_memory_block->p_elements[i].hierarchy_top.pp_members[j]->p_element_type_hierarchy_member->p_actual_type) {
                    msg_stderr(
                            "WARNING: type of the pointer doesn't match with the element found by its address\n");
                }
                /*found - element i + offset - store and return*/
                msg_stderr("found element %d member %d + offset %d\n", (int )i,
                        (int )j, (int )remaining_offset);
                *pp_element = &p_memory_block->p_elements[i];
                *pp_element_hierarchy_member =
                        p_memory_block->p_elements[i].hierarchy_top.pp_members[j];
...
static int object_root_container_memory_block_reference_delete(
    struct object_root_container_memory_block_reference **pp_object_root_container_memory_block_reference) {
_delete_start_(object_root_container_memory_block_reference);
(*pp_object_root_container_memory_block_reference)->p_memory_block_reference =
        NULL;
...
p_object_root_container_memory_block_reference->p_memory_block_reference =
        p_memory_block_reference;
...
struct object_root_container_memory_block_reference *_memory_block_reference,
        *__memory_block_reference;

if (keep_data == 0
        && p_object_root_container_memory_block_top->p_memory_block_top
                != NULL) {
    memory_block_top_delete(
            &p_object_root_container_memory_block_top->p_memory_block_top);
}

cds_list_for_each_entry_safe(_memory_block_reference, __memory_block_reference,
        &p_object_root_container_memory_block_top->references, list)
{
    cds_list_del(&_memory_block_reference->list);
    object_root_container_memory_block_reference_delete(
            &_memory_block_reference);
}
}
...
static int object_root_container_memory_block_top_delete(
    struct object_root_container_memory_block_top **pp_object_root_container_memory_block_top) {
_delete_start_(object_root_container_memory_block_top);
object_root_container_memory_block_top_clean(
        *pp_object_root_container_memory_block_top, 0);
...
if (object_root_container_memory_block_top_init(
        ...
        memory_block_top_create(global_path, p_memory_backend_interface,
                p_element_type, p_array_info)) != 0) {
    object_root_container_memory_block_top_delete(
            &p_object_root_container_memory_block_top);
...
if (p_object_root_builder_memory_block_top_task->global_path == NULL) {
    msg_stderr("wasn't able to duplicate global_path\n");
    object_root_builder_delete_memory_block_top_task(
            &p_object_root_builder_memory_block_top_task);
    return NULL;
}
if (add_traversing_task_to_tail(
...
_memory_block_reference = object_root_container_memory_block_reference_create(
        &p_element_pointer->memory_block_reference);
if (_memory_block_reference == NULL) {
    msg_stderr(
            "wasn't able to create object_root_container_memory_block_reference\n");
    return -(EFAULT);
}
cds_list_add(&_memory_block_reference->list,
        &p_object_root_container_memory_block_top->references);
p_element_pointer->memory_block_reference.p_memory_block_top =
        p_object_root_container_memory_block_top->p_memory_block_top;

/* fill-in from info */
p_element_pointer->memory_block_reference.reference_location.p_element =
        p_element;
p_element_pointer->memory_block_reference.reference_location.p_element_hierarchy_member =
        p_element_hierarchy_member;
...
byte_size = p_element_pointer->p_casted_element_type->byte_size
        * elements_count;
...
cds_list_for_each_entry(p_object_root_container_memory_block_top,
        &p_object_root_builder->container.memory_block_top_list, list)
{
    struct memory_block *p_memory_block =
            &p_object_root_container_memory_block_top->p_memory_block_top->memory_block; //p_object_root_container_memory_block_top->p_memory_block_top->p_memory_block;
    msg_stddbg("COMPARING WITH %p\n", p_memory_block);

    /* hmm.. maybe to put comparison to the driver? */
    if (memory_backend_interface_equals(
            p_memory_block->p_memory_backend_interface,
            p_element_pointer->p_casted_memory_backend_interface)
        ...
        object_root_builder_update_pointer(p_object_root_builder, global_path,
                p_element, p_element_hierarchy_member, p_element_pointer,
                p_element_type_pointer,
                p_object_root_container_memory_block_top);
        return 0;
    }
...
p_object_root_container_memory_block_top =
        object_root_container_memory_block_top_create(global_path,
                p_element_pointer->p_casted_memory_backend_interface,
                p_element_pointer->p_casted_element_type, p_array_info);
...
object_root_builder_update_pointer(p_object_root_builder, global_path,
        p_element, p_element_hierarchy_member, p_element_pointer,
        p_element_type_pointer, p_object_root_container_memory_block_top);

if (object_root_builder_schedule_object_root_container_memory_block_top(
        p_object_root_builder, global_path,
        p_object_root_container_memory_block_top) != 0) {
    msg_stderr(
            "object_root_builder_schedule_object_root_container_memory_block_top failed for %s\n",
            global_path);
    object_root_container_memory_block_top_delete(
            &p_object_root_container_memory_block_top);
    return (-EFAULT);
...
/* add to the list - after this point container will take care of deleting this item */
cds_list_add_tail(&p_object_root_container_memory_block_top->list,
...
for (i = 0;
        i
                < p_object_root_container_memory_block_top->p_memory_block_top->memory_pointers_count;
        ++i) {
    if (object_root_builder_process_pointer(p_object_root_builder, global_path,
            p_object_root_container_memory_block_top->p_memory_block_top->pp_memory_pointers[i]->p_element,
            p_object_root_container_memory_block_top->p_memory_block_top->pp_memory_pointers[i]->p_element_hierarchy_member)
            != 0) {
...
cds_list_add_tail(&p_task->list,
        &p_object_root_builder->object_root_executed_tasks);
if (error_flag != 0)
    return (-EALREADY);
...
return (object_root_builder_schedule_memory_block_top_with_fn(
        ...
        global_path, p_object_root_container_memory_block_top) != NULL) ?
        0 : (-EFAULT);
}
...
struct object_root_container_memory_block_top *_memory_block_top,
        *__memory_block_top;
cds_list_for_each_entry_safe(_memory_block_top, __memory_block_top,
        &p_object_root_container->memory_block_top_list, list)
{
...
static void object_root_builder_clean(
    struct object_root_builder *p_object_root_builder, metac_flag_t keep_data) {
struct traversing_engine_task *p_task, *_p_task;

traversing_engine_clean(&p_object_root_builder->object_root_traversing_engine);
cds_list_for_each_entry_safe(p_task, _p_task,
        &p_object_root_builder->object_root_executed_tasks, list)
{
    struct object_root_builder_memory_block_top_task
    * p_object_root_builder_memory_block_top_task = cds_list_entry(
            p_task,
...
/*get arrays lengths */
cds_list_for_each_entry(_memory_block_top,
        &p_object_root_builder->container.memory_block_top_list, list)
{
...
if (p_object_root->memory_block_tops_count > 0) {
            ...
            &p_object_root_builder->container.memory_block_top_list, list)
    {
        ...
        p_object_root->pp_memory_block_tops[i] =
                _memory_block_top->p_memory_block_top;

        p_object_root->pp_memory_block_tops[i]->id = i;
        p_object_root->pp_memory_block_tops[i]->memory_block_references_count =
                0;
        cds_list_for_each_entry(_memory_block_reference,
                &_memory_block_top->references, list)
        {
            ++p_object_root->pp_memory_block_tops[i]->memory_block_references_count;
        }

        if (p_object_root->pp_memory_block_tops[i]->memory_block_references_count
                > 0) {
            p_object_root->pp_memory_block_tops[i]->pp_memory_block_references =
                    (struct memory_block_reference**) calloc(
                            p_object_root->pp_memory_block_tops[i]->memory_block_references_count,
                            sizeof(*p_object_root->pp_memory_block_tops[i]->pp_memory_block_references));
            if (p_object_root->pp_memory_block_tops[i]->pp_memory_block_references
                    == NULL) {
                msg_stderr(
                        "can't allocate memory for memory_block_references\n");
        ...
        cds_list_for_each_entry(_memory_block_reference,
                &_memory_block_top->references, list)
        {
            p_object_root->pp_memory_block_tops[i]->pp_memory_block_references[j] =
                    _memory_block_reference->p_memory_block_reference;
            ++j;
        }
...
void object_root_clean(struct object_root *p_object_root) {
int i;
/*filled in */
...
int object_root_init(struct object_root *p_object_root,
    struct memory_backend_interface *p_memory_backend_interface,
    struct element_type_top *p_element_type_top) {
metac_array_info_t *p_array_info;
...
p_array_info = metac_array_info_create_from_type(
        p_element_type_top->p_element_type_for_pointer->p_actual_type, 1);
if (p_array_info == NULL) {
    msg_stderr("metac_array_info_create_from_type failed\n");
...
if (memory_block_init(&p_object_root->memory_block_for_pointer, NULL, NULL,
        p_memory_backend_interface,
        p_element_type_top->p_element_type_for_pointer, p_array_info, 0) != 0) {
...
if (object_root_builder_process_pointer(&object_root_builder, "ptr",
        &p_object_root->memory_block_for_pointer.p_elements[0], NULL) != 0) {
    msg_stderr(
            "object_root_builder_process_object_root_container_memory_block failed\n");
    object_root_builder_clean(&object_root_builder, 0);
    object_root_clean(p_object_root);
    return -EFAULT;
}
/*process all scheduled memory blocks*/
if (traversing_engine_run(&object_root_builder.object_root_traversing_engine)
        != 0) {
    msg_stderr("tasks execution finished with fail\n");
    object_root_builder_clean(&object_root_builder, 0);
    object_root_clean(p_object_root);
...
int object_root_free_memory(struct object_root *p_object_root) {
int i;
...
int object_root_copy(struct object_root *p_object_root,
    struct object_root **pp_object_root) {
return object_root_copy_ex(p_object_root,
        p_object_root->p_memory_backend_interface->p_ops, pp_object_root);
}
int object_root_copy_ex(struct object_root *p_object_root,
...
int object_root_equals(struct object_root *p_object_root0,
...
if (object_root_init(&p_metac_runtime_object->object_root,
        p_memory_backend_interface, &p_metac_precompiled_type->element_type_top)
        != 0) {
...
#define NEXT_DUMPPREFIX_FREE(next_prefix) \
		free(next_prefix);
/*****************************************************************************/
void metac_type_ptr_dump(FILE *stream, char *prefix, char *field,
...
void metac_type_dump(FILE *stream, char *prefix,
    struct metac_type *p_metac_type) {
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
fprintf(stream, "%smetac_type: {\n", prefix);
fprintf(stream, "%sid: 0x%x(%s)\n", next_prefix, p_metac_type->id,
        metac_type_id_to_char(p_metac_type->id));
fprintf(stream, "%sname: %s\n", next_prefix, p_metac_type->name);
switch (p_metac_type->id) {
case DW_TAG_pointer_type:
    metac_type_ptr_dump(stream, next_prefix, "pointer_type_info.type: ",
            p_metac_type->pointer_type_info.type);
    break;
case DW_TAG_const_type:
    metac_type_ptr_dump(stream, next_prefix, "const_type_info.type: ",
            p_metac_type->const_type_info.type);
    break;
case DW_TAG_array_type:
    metac_type_ptr_dump(stream, next_prefix, "array_type_info.type: ",
            p_metac_type->array_type_info.type);
    break;
}
fprintf(stream, "%s}\n", prefix);
...
void discriminator_dump(FILE *stream, char *prefix,
    struct discriminator *p_discriminator) {
metac_count_t i;
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
...
fprintf(stream, "%sprecondition.p_discriminator: %p\n", next_prefix,
        p_discriminator->precondition.p_discriminator);
fprintf(stream, "%sprecondition.cb: %p(%s)\n", next_prefix, p_discriminator->cb,
        std_cb_ptr_to_char(p_discriminator->cb));
fprintf(stream, "%sprecondition.p_data: %p\n", next_prefix,
        p_discriminator->p_data);
fprintf(stream, "%s}\n", prefix);
NEXT_DUMPPREFIX_FREE(next_prefix);
}
void element_type_hierarchy_top_discriminators_dump(FILE *stream, char *prefix,
...
void element_type_array_dump(FILE *stream, char *prefix,
    struct element_type_array *p_element_type_array) {
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
fprintf(stream, "%sarray: {\n", prefix);
fprintf(stream, "%sp_element_type: %p\n", next_prefix,
        p_element_type_array->p_element_type);
fprintf(stream, "%sarray_elements_count.cb: %p(%s)\n", next_prefix,
        p_element_type_array->array_elements_count.cb,
        std_cb_ptr_to_char(p_element_type_array->array_elements_count.cb));
fprintf(stream, "%sarray_elements_count.p_data: %p\n", next_prefix,
        p_element_type_array->array_elements_count.p_data);
fprintf(stream, "%s}\n", prefix);
NEXT_DUMPPREFIX_FREE(next_prefix);
}
void generic_cast_type_dump(FILE *stream, char *prefix,
    struct generic_cast_type *generic_cast_type) {
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
fprintf(stream, "%sgeneric_cast_type: {\n", prefix);
fprintf(stream, "%sp_element_type: %p\n", next_prefix,
        generic_cast_type->p_element_type);
metac_type_ptr_dump(stream, next_prefix, "p_type: ", generic_cast_type->p_type);
fprintf(stream, "%s}\n", prefix);
NEXT_DUMPPREFIX_FREE(next_prefix);
}
void generic_cast_types_dump(FILE *stream, char *prefix,
    metac_count_t types_count, struct generic_cast_type *p_generic_cast_type) {
metac_count_t i;
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
...
void element_type_pointer_dump(FILE *stream, char *prefix,
    struct element_type_pointer *p_element_type_pointer) {
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
...
fprintf(stream, "%sgeneric_cast.cb: %p(%s)\n", next_prefix,
        p_element_type_pointer->generic_cast.cb,
        std_cb_ptr_to_char(p_element_type_pointer->generic_cast.cb));
...
if (p_element_type_pointer->generic_cast.p_types) {
    generic_cast_types_dump(stream, next_prefix,
            p_element_type_pointer->generic_cast.types_count,
            p_element_type_pointer->generic_cast.p_types);
}
fprintf(stream, "%sarray_elements_count.cb: %p(%s)\n", next_prefix,
        p_element_type_pointer->array_elements_count.cb,
        std_cb_ptr_to_char(p_element_type_pointer->array_elements_count.cb));
fprintf(stream, "%sarray_elements_count.p_data: %p\n", next_prefix,
        p_element_type_pointer->array_elements_count.p_data);
fprintf(stream, "%s}\n", prefix);
NEXT_DUMPPREFIX_FREE(next_prefix);
}
void element_type_hierarchy_member_dump(FILE *stream, char *prefix,
...
if (p_member->p_type)
    metac_type_ptr_dump(stream, next_prefix, "p_type: ", p_member->p_type);
if (p_member->p_actual_type)
    metac_type_ptr_dump(stream, next_prefix, "p_actual_type: ",
            p_member->p_actual_type);
if (p_member->p_actual_type) {
...
void element_type_hierarchy_top_members_dump(FILE *stream, char *prefix,
...
void element_type_hierarchy_top_dump(FILE *stream, char *prefix,
    struct element_type_hierarchy_top *p_element_type_hierarchy_top) {
metac_count_t i;
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
...
fprintf(stream, "%sdiscriminators_count: %d\n", next_prefix,
        p_element_type_hierarchy_top->discriminators_count);
element_type_hierarchy_top_discriminators_dump(stream, next_prefix,
        p_element_type_hierarchy_top->discriminators_count,
        p_element_type_hierarchy_top->pp_discriminators);
fprintf(stream, "%smembers_count: %d\n", next_prefix,
        p_element_type_hierarchy_top->members_count);
element_type_hierarchy_top_members_dump(stream, next_prefix,
        p_element_type_hierarchy_top->members_count,
        p_element_type_hierarchy_top->pp_members);
fprintf(stream, "%s}\n", prefix);
NEXT_DUMPPREFIX_FREE(next_prefix);
}
void element_type_dump(FILE *stream, char *prefix,
    struct element_type *p_element_type) {
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
fprintf(stream, "%selement_type: {\n", prefix);
if (p_element_type->p_type) {
    metac_type_ptr_dump(stream, next_prefix, "p_type: ",
            p_element_type->p_type);
}
if (p_element_type->p_actual_type) {
    metac_type_ptr_dump(stream, next_prefix, "p_actual_type: ",
            p_element_type->p_actual_type);
}
fprintf(stream, "%sbyte_size: %d\n", next_prefix, p_element_type->byte_size);
fprintf(stream, "%sis_potentially_flexible: %d\n", next_prefix,
        p_element_type->is_potentially_flexible);
if (p_element_type->p_actual_type) {
    switch (p_element_type->p_actual_type->id) {
    case DW_TAG_array_type:
        ...
        break;
    case DW_TAG_pointer_type:
        element_type_pointer_dump(stream, next_prefix,
                &p_element_type->pointer);
        break;
    case DW_TAG_structure_type:
    case DW_TAG_union_type:
        element_type_hierarchy_top_dump(stream, next_prefix,
                &p_element_type->hierarchy_top);
        break;
    }
}
...
void element_type_ptr_dump(FILE *stream, char *prefix, char *field,
    struct element_type *p_element_type) {
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
fprintf(stream, "%s%s%p {\n", prefix, field ? field : "pointer: ",
        p_element_type);
element_type_dump(stream, next_prefix, p_element_type);
fprintf(stream, "%s}\n", prefix);
NEXT_DUMPPREFIX_FREE(next_prefix);
}
void _element_types_array_members_dump(FILE *stream, char *prefix,
    metac_count_t element_types_count, struct element_type **pp_element_types) {
metac_count_t i;
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
...
void element_type_top_dump(FILE *stream, char *prefix,
    struct element_type_top *p_element_type_top) {
NEXT_DUMPPREFIX_ALLOC(next_prefix, prefix);
fprintf(stream, "%selement_type_top: {\n", prefix);
if (p_element_type_top->p_pointer_type) {
    metac_type_ptr_dump(stream, next_prefix, "p_pointer_type: ",
            p_element_type_top->p_pointer_type);
}
if (p_element_type_top->p_element_type_for_pointer) {
    element_type_ptr_dump(stream, next_prefix, "p_element_type_for_pointer: ",
            p_element_type_top->p_element_type_for_pointer);
}
fprintf(stream, "%selement_types_count: %d\n", next_prefix,
        p_element_type_top->element_types_count);
_element_types_array_members_dump(stream, next_prefix,
        p_element_type_top->element_types_count,
        p_element_type_top->pp_element_types);
fprintf(stream, "%s}\n", prefix);
NEXT_DUMPPREFIX_FREE(next_prefix);
}
