...
END {
    ...
    print "#include \"ffi_meta.h\""
    print "\n/* early declaration */"
    for (i in obj) {
	print "static struct ffi_meta_type data_" obj[i] ";";
    }
    print "\n/* real data */"
    for (i in obj) {
	in_task = 0;
	...
	print "/* --" i "--*/"
	if ("child" in data[i]) {
	    print "static struct ffi_meta_type *data_" i "_child[] = {";
	    for (k in data[i]["child"]) print "\t&data_" data[i]["child"][k] ",";
	    print "};"
	}
	at_num = 0;
	print "static struct ffi_meta_type_at data_" i "_at[] = {";
	for (j in data[i]) {
	    at_num += dump_at(i, j);
