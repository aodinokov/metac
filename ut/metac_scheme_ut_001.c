...
START_TEST(metac_scheme_001) {
	...
	fail_unless(metac_hierarchy_top_scheme_get_members_count(p_metac_scheme) == 2, "got %d instead 2\n", metac_hierarchy_top_scheme_get_members_count(p_metac_scheme));

	for (i = 0; i < metac_hierarchy_top_scheme_get_members_count(p_metac_scheme); ++i) {

		struct metac_scheme * p_metac_hierarchy_member_scheme;

		p_metac_hierarchy_member_scheme = metac_hierarchy_top_scheme_get_hierarchy_member_scheme(p_metac_scheme, i);
		fail_unless(p_metac_hierarchy_member_scheme != NULL);

		fail_unless(metac_scheme_is_hierarchy_member_scheme(p_metac_hierarchy_member_scheme) == 1);
		fail_unless(metac_hierarchy_member_scheme_get_parent_scheme(p_metac_hierarchy_member_scheme) == p_metac_scheme);

		switch (i) {
		case 0:
			fail_unless(metac_scheme_is_pointer_scheme(p_metac_hierarchy_member_scheme) == 1);
			break;
		case 1:
			fail_unless(metac_scheme_is_array_scheme(p_metac_hierarchy_member_scheme) == 1);
			break;
		}
