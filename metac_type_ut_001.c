...
METAC_TYPE_IMPORT(func_ptr_t);

/* function */
typedef bit_fields_t * p_bit_fields_t;
METAC_TYPE_IMPORT(p_bit_fields_t);
int_t func_t(p_bit_fields_t arg) {if (arg)return 1; return 0;}
METAC_TYPE_IMPORT(func_t);

typedef const char * cchar_t;
METAC_TYPE_IMPORT(cchar_t);
void func_printf(cchar_t format, ...){return;}
METAC_TYPE_IMPORT(func_printf);
METAC_OBJECT_(func_printf, func_printf);
...
METAC_TYPE_IMPORT(char_array_t);
